...
#+begin_abstract
Las mismas dos ortogonalidades de la lección 7, vistas ahora bajo el teorema de Pitágoras, descomponen la varianza de $\boldsymbol{y}$ en la del ajuste más la del error. El cociente resultante es el cuadrado del coseno del ángulo entre los datos y su ajuste, ambos en desviaciones: el coeficiente de determinación $R^2$.
...
STC = Suma Total de Cuadrados, SEC = Suma Explicada de Cuadrados y  SRC = Suma Residual de Cuadrados.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
Con el triángulo rectángulo ya establecido en la transparencia anterior, aplicar el teorema de Pitágoras[fn::demostrado en la lección 3 para cualquier producto escalar con simetría, linealidad y positividad —propiedades que hereda $\langle\cdot,\cdot\rangle_s$, como se explicitó en la lección 4.] es inmediato:
\[
\|\boldsymbol{y}-\boldsymbol{\mathop{\overline{y}}}\|_s^2 = \|\boldsymbol{\mathop{\widehat{y}}}-\boldsymbol{\mathop{\overline{y}}}\|_s^2 + \|\boldsymbol{\mathop{\widehat{e}}}\|_s^2.
\]
...
\[
\mathrm{STC} = \mathrm{SEC} + \mathrm{SRC};
\]
con $\mathrm{STC}=\sum_i(y_i-\mu_{\boldsymbol{y}})^2,\;$ $\mathrm{SEC}=\sum_i(\hat y_i-\mu_{\boldsymbol{y}})^2\;$ y $\mathrm{SRC}=\sum_i\hat e_i^2$.
...
Para comparar cuánto se parecen $\boldsymbol{y}$ e $\boldsymbol{\mathop{\widehat{y}}}$ basta mirar lo que /no/ comparten.

#+attr_latex: :width \textwidth :position [H]
#+attr_html: :width 700px
...
Las dos lecturas son, por tanto, la misma cosa vista de dos maneras: $R^2$ es, a la vez, un ratio de normas al cuadrado (la forma que motivamos con la figura de la transparencia anterior) y un coseno al cuadrado (la forma habitual en los manuales que conecta directamente con el cuadrado de la correlación entre el vector de datos $\boldsymbol{y}$ y su ajuste $\boldsymbol{\mathop{\widehat{y}}}$). El ángulo $\theta$ es precisamente el ángulo marcado con el arco en la figura de la transparencia 2: el ángulo entre el vector en desviaciones de los datos y el vector en desviaciones de su ajuste: $\theta = \measuredangle\Big((\boldsymbol{y}-\boldsymbol{\mathop{\overline{y}}}),\,(\boldsymbol{\mathop{\widehat{y}}}-\boldsymbol{\mathop{\overline{y}}})\Big)$.
...
Conviene subrayar una diferencia con el ejemplo de correlación tratado en la lección 7. Allí, $\rho_{\boldsymbol{x}\boldsymbol{y}}=\cos\theta'$ era el coseno del ángulo entre los vectores en desviaciones, $\boldsymbol{y}-\boldsymbol{\mathop{\overline{y}}}$ y $\boldsymbol{x}-\boldsymbol{\mathop{\overline{x}}}$, correspondientes a los /datos/ y el _/regresor/_. Aquí, $R^2=\rho_{\boldsymbol{y}\boldsymbol{\mathop{\widehat{y}}}}^2=\cos^2\theta$ es el cuadrado del coseno del ángulo /entre los datos y su _ajuste_/ ($\boldsymbol{y}-\boldsymbol{\mathop{\overline{y}}}$ y $\boldsymbol{\mathop{\widehat{y}}}-\boldsymbol{\mathop{\overline{y}}}$), /ambos en desviaciones/ (si no, no sería una correlación). Son, en principio, dos ejemplos de ángulos distintos, definidos entre parejas de vectores distintas. Sin embargo, en la próxima transparencia veremos que, en el caso particular de la regresión lineal simple (un único regresor $\boldsymbol{x}$ además de la constante), ambos ángulos coinciden exactamente.
...
Todo vector del plano $\mathcal{L}(\boldsymbol{1},\boldsymbol{x})$, una vez centrado, cae en /la misma recta/:
\[
\boldsymbol{w}=c_0\boldsymbol{1}+c_1\boldsymbol{x}
\;\Longrightarrow\;
...
La identidad $\mathrm{STC}=\mathrm{SEC}+\mathrm{SRC}$, que en las transparencias 2-3 demostramos como una única igualdad entre normas de vectores en $\mathbb{R}^n$, se lee aquí observación a observación: la suma de las áreas de la izquierda es igual a la suma de las áreas del centro más la suma de las áreas de la derecha. Es la misma identidad, vista con otros ojos —con los ojos del diagrama de dispersión en lugar de los del espacio abstracto de datos—, y por eso usamos aquí las siglas STC/SEC/SRC (la versión ``sin dividir por $n$'', de la que advertimos en la transparencia 3), pues estamos representando la versión que corresponde a sumar áreas de cuadrados sin más.

Esta segunda representación —cuadrados sobre el diagrama de dispersión frente a vectores abstractos en $\mathbb{R}^n$— es, de hecho, la ilustración clásica que aparece en muchos manuales de econometría al presentar $R^2$ por primera vez; aquí llega en último lugar, precisamente para que el alumno entendiera primero /por qué/ es cierta la identidad (Pitágoras en $\mathbb{R}^n$, transparencias 2-3) antes de verla como una simple receta de ``sumar áreas de cuadraditos'' y cuya representación gráfica solo es viable y legible en el caso de la regresión lineal simple.
...
| Definición                | $R^2={\sigma_{\boldsymbol{\mathop{\widehat{y}}}}^2}/{\sigma_{\boldsymbol{y}}^2}=\mathrm{SEC}/\mathrm{STC}\;(=\cos^2\theta=\rho_{\boldsymbol{y}\boldsymbol{\mathop{\widehat{y}}}}^2)$                                                                       |
...
* Preguntas de repaso (sesión 10)                                  :htmlonly:
   :PROPERTIES:
   :metadata: (slideshow . ((slide_type . skip)))
   :END:
#+attr_ipynb: (slideshow . ((slide_type . skip)))
...
1) $\theta$ se acerca a $0^\circ$ y $R^2$ se acerca a $1$.
2) $\theta$ se acerca a $90^\circ$ y $R^2$ se acerca a $0$.
3) $\theta$ se acerca a $180^\circ$ y $R^2$ se vuelve negativo.
...
2) Dos series pueden tener una correlación alta sin que exista ninguna relación causal entre ellas.
