...
Las dos lecturas son, por tanto, la misma cosa vista de dos maneras: $R^2$ es, a la vez, un ratio de normas al cuadrado (la forma que motivamos con la figura de la transparencia anterior) y un coseno al cuadrado (la forma habitual en los manuales que conecta directamente con el cuadrado de la correlación entre el vector de datos $\boldsymbol{y}$ y su ajuste $\boldsymbol{\mathop{\widehat{y}}}$). El ángulo $\theta$ es precisamente el ángulo marcado con el arco en la figura de la transparencia ``El triángulo rectángulo de la bondad de ajuste'': el ángulo entre el vector en desviaciones de los datos y el vector en desviaciones de su ajuste: $\theta = \measuredangle\Big((\boldsymbol{y}-\boldsymbol{\mathop{\overline{y}}}),\,(\boldsymbol{\mathop{\widehat{y}}}-\boldsymbol{\mathop{\overline{y}}})\Big)$.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
Los tres diagramas de esta figura viven en un espacio distinto del que hemos usado en las demás figuras de esta sesión: no es $\mathbb{R}^n$ (el espacio abstracto de los $n$ datos, donde vive el triángulo de las transparencias ``El triángulo rectángulo de la bondad de ajuste'' y ``Pitágoras: la varianza se descompone''), sino el plano $(x,y)$ empleado en los diagramas de dispersión de toda la vida (y que ya hemos usado en el laboratorio que sigue a la lección 5). Es un complemento de la representación abstracta con vectores: mientras que allí una norma al cuadrado es ``la longitud de un vector, elevada al cuadrado'', aquí cada sumando de STC, SEC o SRC es, literalmente, /el área de un cuadrado/ dibujado sobre el propio diagrama de dispersión.
...
La identidad $\mathrm{STC}=\mathrm{SEC}+\mathrm{SRC}$, que en las transparencias ``El triángulo rectángulo de la bondad de ajuste'' y ``Pitágoras: la varianza se descompone'' demostramos como una única igualdad entre normas de vectores en $\mathbb{R}^n$, se lee aquí observación a observación: la suma de las áreas de la izquierda es igual a la suma de las áreas del centro más la suma de las áreas de la derecha. Es la misma identidad, vista con otros ojos —con los ojos del diagrama de dispersión en lugar de los del espacio abstracto de datos—, y por eso usamos aquí las siglas STC/SEC/SRC (la versión ``sin dividir por $n$'', de la que advertimos en la transparencia ``Pitágoras: la varianza se descompone''), pues estamos representando la versión que corresponde a sumar áreas de cuadrados sin más.

Esta segunda representación —cuadrados sobre el diagrama de dispersión frente a vectores abstractos en $\mathbb{R}^n$— es, de hecho, la ilustración clásica que aparece en muchos manuales de econometría al presentar $R^2$ por primera vez; aquí llega en último lugar, precisamente para que el alumno entendiera primero /por qué/ es cierta la identidad (Pitágoras en $\mathbb{R}^n$, transparencias ``El triángulo rectángulo de la bondad de ajuste'' y ``Pitágoras: la varianza se descompone'') antes de verla como una simple receta de ``sumar áreas de cuadraditos'' y cuya representación gráfica solo es viable y legible en el caso de la regresión lineal simple.
