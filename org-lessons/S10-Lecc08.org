...
\[
\boldsymbol{w}=c_1\boldsymbol{1}+c_2\boldsymbol{x}
\;\Longrightarrow\;
\boldsymbol{w}-\boldsymbol{\mathop{\overline{w}}} = c_2\,(\boldsymbol{x}-\mu_{\boldsymbol{x}}\boldsymbol{1}).
\]
...
2. $R^2=\cos^2\theta$ mide, literalmente, cuánto se parece —en dirección, no en escala— el vector de datos en desviaciones al vector ajustado en desviaciones. En la regresión simple, ese ángulo coincide con el de la correlación entre $\boldsymbol{x}$ e $\boldsymbol{y}$ (transparencia 9), cerrando el círculo abierto en la lección 5; en la regresión múltiple (lección 10) seguirá siendo $\cos^2\theta$, pero ya no será el cuadrado de alguna correlación con un regresor particular.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
/Respuesta correcta: 2./
/Evalúa/: resultado central de la transparencia 9.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
/Respuesta correcta: 2./
/Evalúa/: la propiedad anticipada en la transparencia 8 (sin demostración), motivo de la futura introducción del $R^2$ ajustado. Discrimina porque exige recordar un anticipo, no una demostración desarrollada en esta sesión.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
/Respuesta correcta: 2./
/Evalúa/: comprensión correcta de la advertencia central de la transparencia 8: un $R^2$ (o $\rho$) alto no es evidencia de causalidad; y, además, la identidad de Pitágoras se cumple siempre, con independencia de que la relación tenga o no sentido causal (descarta la opción 4, un distractor tentador).
