...
#+LANGUAGE: es
#+DESCRIPTION: Lección 8: bondad de ajuste. Pitágoras descompone la varianza en STC = SEC + SRC y el R² es el coseno al cuadrado del ángulo entre datos y ajuste.
...
Nos condujeron al hallazgo de dos identidades. Por una parte:
\[
...
/Para profundizar/:
- Wooldridge, J. M. (2020). /Introductory Econometrics/, cap. 2, sección 2.3: ``Properties of OLS on Any Sample of Data'', con la presentación clásica de $\mathrm{SST}=\mathrm{SSE}+\mathrm{SSR}$.
...
#+attr_ipynb: (slideshow . ((slide_type . fragment)))
/Versión interactiva/: [[https://mybinder.org/v2/gh/mbujosab/PEconometria/gh-pages?labpath=CuadernosElectronicos/S10-Notebook03.ipynb][cuaderno 3 en MyBinder]], parte 1 (el triángulo, rotable).
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
Con el triángulo rectángulo ya establecido en la transparencia ``El triángulo rectángulo de la bondad de ajuste'', aplicar el teorema de Pitágoras[fn::demostrado en la lección 3 para cualquier producto escalar con simetría, linealidad y positividad, propiedades que hereda $\langle\cdot,\cdot\rangle_s$, como se explicitó en la lección 4.] es inmediato:
\[
\|\boldsymbol{y}-\boldsymbol{\mathop{\overline{y}}}\|_s^2 = \|\boldsymbol{\mathop{\widehat{y}}}-\boldsymbol{\mathop{\overline{y}}}\|_s^2 + \|\boldsymbol{\mathop{\widehat{e}}}\|_s^2.
\]
...
\]
donde, como $\boldsymbol{\mathop{\widehat{e}}}$ tiene media cero, $\mu_{\boldsymbol{\mathop{\widehat{e}}}}=0$ (primera condición de ortogonalidad de la lección 7), su varianza $\sigma_{\boldsymbol{\mathop{\widehat{e}}}}^2$ coincide con $\|\boldsymbol{\mathop{\widehat{e}}}\|_s^2$, puesto que $\boldsymbol{\mathop{\overline{\widehat{e}}}}=\boldsymbol{0}$. 

Esa es la razón por la que los tres vectores involucrados en el triángulo rectángulo: $(\boldsymbol{y}-\boldsymbol{\mathop{\overline{y}}})$, $(\boldsymbol{\mathop{\widehat{y}}}-\boldsymbol{\mathop{\overline{y}}})$ y $\boldsymbol{\mathop{\widehat{e}}}$, aparecen, en la figura de la transparencia ``El triángulo rectángulo de la bondad de ajuste'', en el plano $\mathcal{L}(\boldsymbol{1})^\perp$ perpendicular a la recta $\mathcal{L}(\boldsymbol{1})$ (los tres pertenecen al plano de aquellos vectores que tienen media cero).
...
/Para profundizar/:
- Wooldridge, J. M. (2020), cap. 2, sección 2.3: definición de SST, SSE y SSR y comentario sobre la nomenclatura.
...
Esto sugiere de manera natural cómo medir la bondad del ajuste: comparando las longitudes de esos dos vectores en desviaciones, por ejemplo, mirando el cociente
\[
\frac{\|\boldsymbol{\mathop{\widehat{y}}}-\boldsymbol{\mathop{\overline{y}}}\|_s^2}{\|\boldsymbol{y}-\boldsymbol{\mathop{\overline{y}}}\|_s^2}
\]
(elevado al cuadrado, para trabajar con varianzas en lugar de con desviaciones típicas). Si $\boldsymbol{\mathop{\widehat{e}}}$ es pequeño (figura de abajo), las dos longitudes son casi iguales, y el cociente está cerca de $1$: buen ajuste. Si $\boldsymbol{\mathop{\widehat{e}}}$ es grande (figura de arriba), $\boldsymbol{\mathop{\widehat{y}}}-\boldsymbol{\mathop{\overline{y}}}$ es mucho más corto que $\boldsymbol{y}-\boldsymbol{\mathop{\overline{y}}}$, y el cociente se hace mucho más pequeño que $1$: mal ajuste. En nuestra figura, por tanto, /el ajuste de abajo es mejor que el de arriba/. El cociente que acabamos de construir lo confirmará numéricamente en la transparencia ``El coeficiente de determinación $R^2$'', donde por fin lo llamamos por su nombre.
...
#+attr_ipynb: (slideshow . ((slide_type . fragment)))
Del triángulo rectángulo, como $\langle\boldsymbol{y}-\boldsymbol{\mathop{\overline{y}}},\boldsymbol{\mathop{\widehat{y}}}-\boldsymbol{\mathop{\overline{y}}}\rangle_s = \|\boldsymbol{\mathop{\widehat{y}}}-\boldsymbol{\mathop{\overline{y}}}\|_s^2\;$ (pues $\boldsymbol{\mathop{\widehat{e}}}\perp\mathcal{L}(\boldsymbol{1},\boldsymbol{x})$):
$\quad\cos\theta = \|\boldsymbol{\mathop{\widehat{y}}}-\boldsymbol{\mathop{\overline{y}}}\|_s/\|\boldsymbol{y}-\boldsymbol{\mathop{\overline{y}}}\|_s = \sigma_{\boldsymbol{\mathop{\widehat{y}}}}/\sigma_{\boldsymbol{y}} = \rho_{\boldsymbol{\mathop{\widehat{y}}}\boldsymbol{y}}$.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
La transparencia ``¿Qué ajuste es mejor: el de arriba o el de abajo?'' nos dejó el camino casi completo: la bondad del ajuste se puede medir comparando las longitudes de los vectores en desviaciones $\boldsymbol{y}-\boldsymbol{\mathop{\overline{y}}}$ y $\boldsymbol{\mathop{\widehat{y}}}-\boldsymbol{\mathop{\overline{y}}}$. Formalizamos esa intuición como definición:
...
\]
ya que, como $\mathrm{STC}=\mathrm{SEC}+\mathrm{SRC}$, podemos sustituir $\mathrm{SEC}=\mathrm{STC}-\mathrm{SRC}$.[fn::Fíjese que $\frac{\mathrm{SEC}}{\mathrm{STC}}=\frac{\|\boldsymbol{\mathop{\widehat{y}}}-\boldsymbol{\mathop{\overline{y}}}\|_e^2}{\|\boldsymbol{y}-\boldsymbol{\mathop{\overline{y}}}\|_e^2}$ es el cociente de normas al cuadrado de los mismos vectores en desviaciones, pero medidos usando el producto escalar habitual $\langle\cdot,\cdot\rangle_e$ del espacio euclídeo $\mathbb{R}^n$.]

Esta definición como cociente de normas admite una segunda lectura, tan reveladora como la primera: es, literalmente, el coseno al cuadrado de un ángulo. Recordemos la definición de coseno entre dos vectores (lección 3) aplicada aquí a los vectores en desviaciones de $\boldsymbol{y}$ y $\boldsymbol{\mathop{\widehat{y}}}$ (como se hizo con $\boldsymbol{x}$ e $\boldsymbol{y}$ en la lección 5): el ángulo $\theta$ entre $\boldsymbol{y}-\boldsymbol{\mathop{\overline{y}}}$ y $\boldsymbol{\mathop{\widehat{y}}}-\boldsymbol{\mathop{\overline{y}}}$ cumple
\begin{equation}
\cos\theta \;=\; \frac{\langle\boldsymbol{y}-\boldsymbol{\mathop{\overline{y}}},\,\boldsymbol{\mathop{\widehat{y}}}-\boldsymbol{\mathop{\overline{y}}}\rangle_s}{\|\boldsymbol{y}-\boldsymbol{\mathop{\overline{y}}}\|_s\,\|\boldsymbol{\mathop{\widehat{y}}}-\boldsymbol{\mathop{\overline{y}}}\|_s} \;=\; \rho_{\boldsymbol{\mathop{\widehat{y}}}\boldsymbol{y}};
\label{eq:coseno-correlacion}
\end{equation}
donde hemos usado la definición de correlación de la lección 5.

Para simplificar el numerador de (\ref{eq:coseno-correlacion}), sustituimos $\boldsymbol{y}-\boldsymbol{\mathop{\overline{y}}}=(\boldsymbol{\mathop{\widehat{y}}}-\boldsymbol{\mathop{\overline{y}}})+\boldsymbol{\mathop{\widehat{e}}}$ (transparencia ``El triángulo rectángulo de la bondad de ajuste''):
\[
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
Las figuras anteriores permiten comprender, sin necesidad de ninguna fórmula adicional, cuándo se dan los casos extremos.
...
$R^2=\mathrm{SEC}/\mathrm{STC}=90/100=0{,}9$: el ajuste reproduce el $90\,\%$ de la variación total de $\boldsymbol{y}$.

Verifiquemos también la vía de la correlación (transparencia ``El coeficiente de determinación $R^2$''). De la lección 7 ya teníamos $\sigma_{\boldsymbol{x}}^2=2$ y $\sigma_{\boldsymbol{x}\boldsymbol{y}}=6$. Y $\sigma_{\boldsymbol{y}}^2=\mathrm{STC}/n=100/5=20$. Entonces
\[
...
\]
Coincide con $R^2$: una comprobación numérica anticipada del resultado general $R^2=\rho_{\boldsymbol{x}\boldsymbol{y}}^2$, que demostraremos formalmente más adelante, en la transparencia ``En el caso de la regresión simple: $R^2=\rho_{\boldsymbol{x}\boldsymbol{y}}^2$''.

# /Para profundizar/:
# - Ninguna referencia adicional; este ejemplo es puramente ilustrativo de las fórmulas ya citadas en las transparencias ``Pitágoras: la varianza se descompone'' a ``El coeficiente de determinación''.
...
/Segundo peligro, distinto del anterior: incluso cuando la relación es real, el $R^2$ no dice nada sobre su dirección/. Retomemos el ejemplo del precio y la superficie de las viviendas. Aquí sí existe una relación económicamente razonable: ampliar la superficie de una vivienda tiende a aumentar su precio. Si regresamos precio sobre superficie, obtenemos un $R^2$ que, en efecto, sugiere que el modelo parece ``explicar'' el precio a partir del tamaño. Pero si invertimos los papeles, regresando /superficie/ sobre /precio/, obtenemos el mismo $R^2$. La razón la veremos en la transparencia ``En el caso de la regresión simple: $R^2=\rho_{\boldsymbol{x}\boldsymbol{y}}^2$'': en la regresión lineal simple $R^2=\rho_{\boldsymbol{x}\boldsymbol{y}}^2$, y la correlación no distingue cuál de las dos variables hace de $x$ y cuál de $y$. Y sin embargo, decir que ``el precio explica la superficie'' (en el sentido de que la subida de los precios hace crecer las superficies) no tiene ningún sentido. La regresión, y el $R^2$ que de ella se deriva, son completamente indiferentes a cuál de las dos variables juega el papel de causa y cuál el de efecto: esa es una pregunta que solo puede responder una teoría económica previa, nunca la mera geometría de los datos.
...
- Wooldridge, J. M. (2020), cap. 1, sección 1.4 (causalidad y /ceteris paribus/) y cap. 2, sección 2.3 (qué mide y qué no mide $R^2$).
...
El ajuste $\boldsymbol{\mathop{\widehat{y}}}=\hat\beta_2\boldsymbol{x}+\hat\beta_1\boldsymbol{1}$ es una /traslación y reescalado/ de $\boldsymbol{x}$. Aplicando las propiedades de traslación y escala de la correlación:
\[
\rho_{\boldsymbol{\mathop{\widehat{y}}}\boldsymbol{y}} \;=\; \frac{\hat\beta_2}{|\hat\beta_2|}\;\rho_{\boldsymbol{x}\boldsymbol{y}}.
\]
...
Empecemos por un hecho general sobre el plano $\mathcal{L}(\boldsymbol{1},\boldsymbol{x})$, que no depende en absoluto de cuál sea el ajuste MCO. Tomemos /cualquier/ vector del plano, es decir, cualquier $\boldsymbol{w}=c_1\boldsymbol{1}+c_2\boldsymbol{x}$. Su media es $\mu_{\boldsymbol{w}}=c_1+c_2\mu_{\boldsymbol{x}}$, así que su vector en desviaciones es
\[
\boldsymbol{w}-\boldsymbol{\mathop{\overline{w}}} = \big(c_1\boldsymbol{1}+c_2\boldsymbol{x}\big) - \big(c_1+c_2\mu_{\boldsymbol{x}}\big)\boldsymbol{1} = c_2\big(\boldsymbol{x}-\mu_{\boldsymbol{x}}\boldsymbol{1}\big).
\]
...
Podemos evitar repetir el argumento desde cero: basta combinar las dos propiedades demostradas en la lección 5 bajo el nombre de /propiedades de traslación y escala/ de la correlación. Allí vimos que, si $\boldsymbol{x}'=\lambda\boldsymbol{x}+a\boldsymbol{1}$ (una traslación seguida de un reescalado), entonces
\[
...
\[
\rho_{\boldsymbol{\mathop{\widehat{y}}}\boldsymbol{y}} = \mathrm{sign}(\rho_{\boldsymbol{x}\boldsymbol{y}})\rho_{\boldsymbol{x}\boldsymbol{y}} = |\rho_{\boldsymbol{x}\boldsymbol{y}}|.
\]
Elevando al cuadrado, y usando $R^2=\rho_{\boldsymbol{\mathop{\widehat{y}}}\boldsymbol{y}}^2$ (transparencia ``El coeficiente de determinación''):
\[
...
/Para profundizar/:
- Wooldridge, J. M. (2020), cap. 2, sección 2.3: $R^2$ como cuadrado de la correlación muestral en el modelo de regresión simple.
...
#+attr_ipynb: (slideshow . ((slide_type . fragment)))
/Versión interactiva/: [[https://mybinder.org/v2/gh/mbujosab/PEconometria/gh-pages?labpath=CuadernosElectronicos/S10-Notebook03.ipynb][cuaderno 3 en MyBinder]], parte 2 (las tres áreas, con datos editables).
...
En el panel central, los cuadrados tienen como lado la distancia vertical entre cada dato y su propio ajuste, $|y_i-\hat y_i|$, es decir, el residuo $|\hat e_i|$; son notablemente más pequeños que en los otros dos paneles cuando el ajuste es razonablemente bueno. La suma de estas áreas es $\mathrm{SRC}=\sum_i\hat e_i^2$.
...
#+attr_ipynb: (slideshow . ((slide_type . fragment)))
*Más adelante:*
el mismo argumento (Pitágoras y las dos ortogonalidades de la lección 7) se generalizará a $k$ regresores: solo cambiará la dimensión del subespacio sobre el que proyectamos. Las primeras figuras de esta sesión, al no mostrar $\hat\beta_2\boldsymbol{x}$ explícitamente, valdrán como esquema para el caso general. 
...
#+attr_ipynb: (slideshow . ((slide_type . fragment)))
/Lección 9 — El puente/: hasta ahora, pura geometría en $\mathbb{R}^n$, sin ningún supuesto probabilístico. La próxima lección teórica da el salto al espacio de las variables aleatorias, como analogía y no como demostración: la esperanza condicional $E[Y\mid X]$ jugará el papel de $\boldsymbol{\mathop{\widehat{y}}}$.
...
2. $R^2=\cos^2\theta$ mide, literalmente, cuánto se parece, en dirección y no en escala, el vector de datos en desviaciones al vector ajustado en desviaciones. En la regresión simple, ese ángulo coincide con el de la correlación entre $\boldsymbol{x}$ e $\boldsymbol{y}$ (transparencia ``En el caso de la regresión simple: $R^2=\rho_{\boldsymbol{x}\boldsymbol{y}}^2$''), cerrando el círculo abierto en la lección 5; en la regresión múltiple (lección 10) seguirá siendo $\cos^2\theta$, pero ya no será el cuadrado de alguna correlación con un regresor particular.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
/Respuesta correcta: 2./
/Evalúa/: la trampa terminológica señalada explícitamente en la transparencia ``Pitágoras: la varianza se descompone'': ``SSE'' no es ``suma de errores'', sino ``suma explicada''.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
/Respuesta correcta: 2./
/Evalúa/: resultado central de la transparencia ``En el caso de la regresión simple: $R^2=\rho_{\boldsymbol{x}\boldsymbol{y}}^2$''.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
/Respuesta correcta: 2./
/Evalúa/: comprensión de que la descomposición solo funciona porque, gracias a la primera condición de ortogonalidad (lección 7), $\boldsymbol{y}$ y $\boldsymbol{\mathop{\widehat{y}}}$ comparten la misma proyección $\boldsymbol{\mathop{\overline{y}}}$ sobre $\mathcal{L}(\boldsymbol{1})$; sin esa igualdad, la identidad de Pitágoras no se podría construir con esos tres vectores.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
/Respuesta correcta: 2./
/Evalúa/: la propiedad anticipada en la transparencia ``Una advertencia: el nombre /suma explicada/ es engañoso'' (sin demostración), motivo de la futura introducción del $R^2$ ajustado. Discrimina porque exige recordar un anticipo, no una demostración desarrollada en esta sesión.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
/Respuesta correcta: 2./
/Evalúa/: comprensión correcta de la advertencia central de la transparencia ``Una advertencia: el nombre /suma explicada/ es engañoso'': un $R^2$ (o $\rho$) alto no es evidencia de causalidad; y, además, la identidad de Pitágoras se cumple siempre, con independencia de que la relación tenga o no sentido causal (descarta la opción 4, un distractor tentador).
