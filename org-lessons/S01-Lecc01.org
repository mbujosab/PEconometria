...
* Preguntas de repaso (sesión 1)                                   :htmlonly:
   :PROPERTIES:
   :metadata: (slideshow . ((slide_type . skip)))
   :END:
#+attr_ipynb: (slideshow . ((slide_type . skip)))
...
1) Demostrar matemáticamente las teorías económicas.
2) Medir cuantitativamente relaciones entre variables económicas a partir de datos.
3) Predecir con certeza el comportamiento futuro de la economía.
4) Sustituir a la teoría económica mediante el análisis de datos.
...
1) La policía causa criminalidad.
2) La criminalidad reduce la policía.
3) Aunque se observe dicha asociación, ello no implica ni que la policía cause el crimen; ni tampoco que exista una causalidad en el sentido inverso; podría ser que ninguna de las dos cause a la otra, por ejemplo, que exista un tercer factor que influye sobre ambas variables.
4) La asociación es probablemente un error de medición.
...
1) Una sección cruzada de 300 observaciones.
2) Una serie temporal de 300 observaciones.
3) Un panel con $i$ = mes y $t$ = año.
...
1) En datos de sección cruzada, reordenar el índice $i$ destruye información esencial.
2) En series temporales, el orden del índice $t$ contiene información relevante que la geometría de $\mathbb{R}^n$ no aprovecha.
3) En los datos de panel basta con un único índice.
4) En sección cruzada, el orden inicial del índice debe ser alfabético.
...
1) El orden de $y$ y el de $x$ pueden fijarse de forma independiente.
2) El orden inicial es arbitrario, pero una vez fijado debe ser /el mismo/ en ambas variables: $y_i$ y $x_i$ deben referirse a la misma persona.
3) Como el orden no contiene información, se puede desordenar $x$ sin que afecte al análisis.
4) Solo $y$ debe estar ordenada; $x$ puede estar en cualquier orden.
...
1) Generalmente no introduce ningún problema, pues el índice temporal no suele ser información relevante para explicar los datos.
2) Es razonable como aproximación, pero ignora la información del orden temporal y puede ocasionar problemas en el análisis.
3) Es incorrecta: jamás debe hacerse.
4) Equivale a convertir las series temporales en datos de panel.
...
1) Únicamente de la estadística.
2) Únicamente de la teoría económica.
3) De teoría económica, estadística/probabilidad y matemáticas, complementadas con software.
4) De la informática y la inteligencia artificial.
...
1) Una relación causal directa entre luz y miopía.
2) Una asociación espuria debida a un factor común no observado (miopía de los padres).
3) Un error de medición en la variable ``luz''.
4) Causalidad inversa: la miopía causa que se deje la luz encendida.
