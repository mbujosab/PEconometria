...
\[
\sigma_{\boldsymbol{y}}^2 =
\|\boldsymbol{y}-\boldsymbol{\mathop{\overline{y}}}\|_s^2 = \langle\boldsymbol{y}-\boldsymbol{\mathop{\overline{y}}},\,\boldsymbol{y}-\boldsymbol{\mathop{\overline{y}}}\rangle_s = \frac{1}{n}\sum_{i=1}^n(y_i-\mu_{\boldsymbol{y}})^2.
\]
...
#+attr_ipynb: (slideshow . ((slide_type . fragment)))
Como $\|\boldsymbol{1}\|_s=1$: $\quad\|\mu_{\boldsymbol{x}}\boldsymbol{1}\|_s = |\mu_{\boldsymbol{x}}|\cdot\|\boldsymbol{1}\|_s = |\mu_{\boldsymbol{x}}| \;\Longrightarrow\; \|\mu_{\boldsymbol{x}}\boldsymbol{1}\|_s^2=\mu_{\boldsymbol{x}}^2$.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
La figura retoma la escena de la lección 4 (dos vectores en $\mathbb{R}^3$ proyectados sobre $\mathcal{L}(\boldsymbol{1})$), pero añade un elemento nuevo: el ángulo $\theta$ entre los vectores en desviaciones $\boldsymbol{v}-\boldsymbol{\mathop{\overline{v}}}$ y $\boldsymbol{x}-\boldsymbol{\mathop{\overline{x}}}$, ambos en el plano $\mathcal{L}(\boldsymbol{1})^\perp$ (perpendicular al eje azul). El coseno de dicho ángulo es la correlación $\rho_{\boldsymbol{v}\boldsymbol{x}}$.
...
Una observación: para que dos vectores en desviaciones puedan formar un ángulo distinto de $0^o$ o $180^o$, necesitamos que $\mathcal{L}(\boldsymbol{1})^\perp$ tenga al menos dos dimensiones, es decir, necesitamos una cantidad de datos $n\geq3$ en cada vector. La siguiente sección o transparencia lo explica. 
...
Verifiquémoslo con dos de los ejemplos numéricos de la lección 4. Tomamos $\boldsymbol{x}=(2,4)$ (con $\mu_{\boldsymbol{x}}=3$, en desviaciones $(-1,1)$ y $\sigma_{\boldsymbol{x}}=1$) y $\boldsymbol{z}=(2,-6)$ (con $\mu_{\boldsymbol{z}}=-2$, en desviaciones $(4,-4)$ y $\sigma_{\boldsymbol{z}}=4$). La covarianza es
\[
...
/Homogeneidad ante escala/. Si $\boldsymbol{x}'=\lambda\boldsymbol{x}$ con $\lambda\neq0$, entonces $\mu_{\boldsymbol{x}'}=\lambda\mu_{\boldsymbol{x}}$; pero ahora, además, el vector en desviaciones también se escala por $\lambda$:
\[
...
\]
Y como $\|\lambda(\boldsymbol{x}-\boldsymbol{\mathop{\overline{x}}})\|_s = |\lambda|\,\|\boldsymbol{x}-\boldsymbol{\mathop{\overline{x}}}\|_s$ (homogeneidad de la norma, lección 2), la correlación queda afectada del siguiente modo:
\[
...
\]
Si $\lambda>0$, $\frac{\lambda}{|\lambda|}=1$ y la correlación no cambia. Si $\lambda<0$, $\frac{\lambda}{|\lambda|}=-1$ y la correlación cambia de signo, pero conserva su magnitud $|\rho_{\boldsymbol{x}\boldsymbol{y}}|=|\rho_{\boldsymbol{x}'\boldsymbol{y}}|$.
...
El valor $\rho_{\boldsymbol{x}\boldsymbol{y}}=\frac{\sqrt3}{2}$ corresponde a un ángulo $\theta=30^o$ entre los vectores en desviaciones $(-1,0,1)$ y $(-1,-1,2)$: ni ortogonales ($\theta\neq90^o$, $\rho\neq0$), ni alineados ($\theta\neq0^o,180^o$, $\rho\neq\pm1$). Es exactamente el tipo de situación que, como vimos en la transparencia anterior, es /imposible/ de obtener con solo $n=2$ observaciones (por eso hemos acudido a un ejemplo en $\mathbb{R}^3$).
...
#+attr_ipynb: (slideshow . ((slide_type . fragment)))
Cuando organicemos los regresores[fn::regresor: cada vector que multiplica a un coeficiente $\beta_j$.] como columnas de una matriz $\boldsymbol{\mathsf{X}}=[\boldsymbol{1};\;\boldsymbol{x}]$, el producto $\boldsymbol{\mathsf{X}}\boldsymbol{\beta}$ admitirá una doble lectura: como una lista de productos escalares, o como una combinación lineal de las columnas de $\boldsymbol{\mathsf{X}}$. Esta doble lectura no es nueva: ya apareció, en su versión más simple, con la propia media (lección 4): $\mu_{\boldsymbol{y}}$ era, a la vez, el producto escalar $\langle\boldsymbol{y},\boldsymbol{1}\rangle_s$ y el coeficiente de la combinación lineal (proyección) $\mu_{\boldsymbol{y}}\boldsymbol{1}$. No obstante, esa notación matricial llegará más adelante; hoy solo la anunciamos.
...
Por último, una pincelada de notación que solo cobrará su forma completa en la lección 10: cuando tengamos varios regresores (por ejemplo, $\boldsymbol{1}$ y $\boldsymbol{x}$), resultará cómodo organizarlos como las columnas de una única matriz $\boldsymbol{\mathsf{X}}=[\boldsymbol{1};\;\boldsymbol{x}]$. El producto $\boldsymbol{\mathsf{X}}\boldsymbol{\beta}$ (con $\boldsymbol{\beta}=(\beta_1,\beta_2)$) admite entonces una doble lectura, que usaremos constantemente a partir de ese momento: por un lado, como la lista de los productos escalares de $\boldsymbol{\beta}$ con cada /fila/ de $\boldsymbol{\mathsf{X}}$; por otro, como la /combinación lineal/ $\beta_1\boldsymbol{1}+\beta_2\boldsymbol{x}$ de las /columnas/ de $\boldsymbol{\mathsf{X}}$. Hoy solo sembramos la idea; la notación matricial completa esperará hasta que de verdad la necesitemos.


* Preguntas de repaso (sesión 6)                                   :htmlonly:
   :PROPERTIES:
   :metadata: (slideshow . ((slide_type . skip)))
   :END:
#+attr_ipynb: (slideshow . ((slide_type . skip)))
