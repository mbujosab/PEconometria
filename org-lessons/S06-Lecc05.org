...
#+LANGUAGE: es
#+DESCRIPTION: Lección 5: la varianza como cuadrado de una norma, la covarianza como producto escalar de vectores centrados y la correlación como coseno de un ángulo.
...
Segundo, vamos a dejar de trabajar con un único vector y vamos a contemplar /dos/ vectores de datos simultáneamente, $\boldsymbol{x}$ e $\boldsymbol{y}$; esto es lo verdaderamente nuevo. Cuando tenemos dos vectores, dos preguntas geométricas naturales son: ¿cuánto vale el producto escalar de sus vectores en desviaciones (sus componentes variables)? y ¿qué ángulo forman dichas componentes variables? Las respuestas son, respectivamente, la /covarianza/ y la /correlación/.
...
/Para profundizar/:
- Wooldridge, J. /Introductory Econometrics/ (2020), Apéndice B, secciones B.3 y B.4: repaso de varianza, covarianza y correlación desde la estadística clásica (sin la lectura geométrica que estamos construyendo aquí).
...
\]
Esta forma alternativa de escribir la varianza es la que los cursos de estadística descriptiva presentan como ``fórmula de cálculo'' o ``fórmula abreviada''. No es un truco algebraico: es lo que se obtiene del teorema de Pitágoras si se despeja la varianza. Para poder calcularla solo falta saber qué es $\|\boldsymbol{y}\|_s^2$ en forma de sumatorios o de medias aritméticas; y eso es justamente lo que hace la transparencia ``Varianza y Pitágoras''.
...
\[
\underbrace{\|\boldsymbol{y}\|_s^2}_{\text{hipotenusa}^2} \;=\; \underbrace{\|\mu_{\boldsymbol{y}}\boldsymbol{1}\|_s^2}_{\text{cateto}^2} \;+\; \underbrace{\sigma_{\boldsymbol{y}}^2}_{\text{cateto}^2}.
\]
...
#+attr_ipynb: (slideshow . ((slide_type . fragment)))
Despejando $\sigma_{\boldsymbol{y}}^2$ y recordando que $\|\boldsymbol{y}\|_s^2=\langle\boldsymbol{y},\boldsymbol{y}\rangle_s$:
\[
\boxed{\;\sigma_{\boldsymbol{y}}^2 \;=\; \|\boldsymbol{y}\|_s^2 - \mu_{\boldsymbol{y}}^2 \;=\; \frac{1}{n}\sum_{i=1}^n y_i^2 \,-\, \mu_{\boldsymbol{y}}^2 \;=\;
\mu_{\boldsymbol{y}^2}  \,-\, \mu_{\boldsymbol{y}}^2;\;}
\]
donde $\boldsymbol{y}^2$ denota el vector cuyas componentes son el cuadrado de las de $\boldsymbol{y}$.
...
Una advertencia importante, que retomaremos en la transparencia ``La correlación: el coseno de un ángulo'': la covarianza, por sí sola, no tiene una escala interpretable. Su valor numérico depende de las unidades en que midamos $\boldsymbol{x}$ e $\boldsymbol{y}$ (si $\boldsymbol{x}$ está en euros y lo pasamos a céntimos, la covarianza se multiplica por 100, sin que la naturaleza de la posible relación entre ambas variables haya cambiado en absoluto, pues solo hemos cambiado la unidad de medida). Esto es lo mismo que le ocurre al producto escalar ordinario: por sí solo no nos dice nada sobre el /ángulo/ entre dos vectores, salvo que lo dividamos por sus normas; lo que produce un cociente sin unidades de medida (pues las unidades del numerador y del denominador se cancelan). Ese es el papel que va a jugar la correlación.
...
#+attr_ipynb: (slideshow . ((slide_type . fragment)))
- $\rho_{\boldsymbol{x}\boldsymbol{y}}=\pm1$: vectores en desviaciones /alineados/ ($\theta=0^\circ$ o $180^\circ$).
- $\rho_{\boldsymbol{x}\boldsymbol{y}}=0$: vectores en desviaciones /ortogonales/ ($\theta=90^\circ$).
...
Es, literalmente, el coseno del ángulo $\theta$ que forman los dos vectores en desviaciones. La desigualdad de Cauchy–Schwarz (lección 3) nos garantiza que $-1\leq\rho_{\boldsymbol{x}\boldsymbol{y}}\leq1$ siempre, para cualesquiera $\boldsymbol{x}$ e $\boldsymbol{y}$.
...
Una observación: para que dos vectores en desviaciones puedan formar un ángulo distinto de $0^\circ$ o $180^\circ$, necesitamos que $\mathcal{L}(\boldsymbol{1})^\perp$ tenga al menos dos dimensiones, es decir, necesitamos una cantidad de datos $n\geq3$ en cada vector. La transparencia ``Un hecho geométrico: en $\mathbb{R}^2$ la correlación es siempre $\pm1$'' lo explica. 
...
El subespacio $\mathcal{L}(\boldsymbol{1})^\perp$ es el conjunto de todos los vectores ortogonales a $\boldsymbol{1}$, es decir, de todos los posibles vectores en desviaciones. Tiene dimensión $n-1$ dentro de $\mathbb{R}^n$ (porque $\mathcal{L}(\boldsymbol{1})$, al ser una recta, ya ``ocupa'' una dimensión).[fn::/Nota sobre la palabra ``dimensión''/. En este curso no hemos
...
/Para profundizar/:
- Strang, G. /Introduction to Linear Algebra/ (2016), sección 3.4 (/Independence, Basis and Dimension/): dimensión de subespacios y su relación con el número de direcciones independientes (tratamiento formal de la noción que aquí usamos de manera informal).
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
La primera de estas dos propiedades es la versión, para dos vectores, de la que ya vimos en la lección 4 para la desviación típica de un solo vector (sumar una constante no la cambia). La segunda es nueva: la lección 4 no trató el cambio de escala.
...
\]
Como la covarianza y la correlación solo dependen de los vectores en desviaciones, ni $\sigma_{\boldsymbol{x}'\boldsymbol{y}}$ ni $\rho_{\boldsymbol{x}'\boldsymbol{y}}$ cambian. Geométricamente: $a\boldsymbol{1}$ es un vector de $\mathcal{L}(\boldsymbol{1})$, así que sumarlo cambia la componente de $\boldsymbol{x}$ en $\mathcal{L}(\boldsymbol{1})$ (su media) y deja intacta su componente en $\mathcal{L}(\boldsymbol{1})^\perp$ (su vector en desviaciones).
...
/Caso particular: la varianza/. Tomando $\boldsymbol{y}=\boldsymbol{x}$ en las dos propiedades anteriores obtenemos, de forma inmediata, las propiedades correspondientes de la varianza (el caso $\sigma_{\boldsymbol{x}\boldsymbol{x}}=\sigma_{\boldsymbol{x}}^2$ que ya vimos en la transparencia de la covarianza). De la invarianza ante traslaciones:
\[
...
La primera identidad ya estaba, en realidad, en la lección 4 (sección ``Propiedades geométricas''): allí vimos que sumar una constante no cambia la desviación típica $\sigma_{\boldsymbol{x}}$, lo cual, al elevar al cuadrado, es exactamente la invarianza de la varianza ante traslaciones. La segunda es nueva, y lo que importa de ella es el efecto, /cuadrático/, de la escala: si $\boldsymbol{x}$ pasa de medirse en euros a medirse en céntimos ($\lambda=100$), la desviación típica se multiplica por $100$, pero la varianza se multiplica por $100^2=10\,000$, no por $100$. Confundir ambos efectos es una fuente frecuente de errores al comparar varianzas expresadas en unidades distintas.
...
/Para profundizar/:
- Wooldridge, J. /Introductory Econometrics/ (2020), Apéndice B, secciones B.3 y B.4: efecto de las transformaciones lineales sobre la media, la varianza y la covarianza.
