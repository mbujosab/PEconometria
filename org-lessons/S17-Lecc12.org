...
Conviene señalar también que la lección 11 ya nos dio, sin que lo pareciera, la pieza que hoy resultará decisiva. Allí escribimos $\hat\beta_2=\beta_2\,\mathit1+\sum_iW_iU_i$, y esa descomposición no era un paso intermedio prescindible: dice que el error de estimación $\hat\beta_2-\beta_2$ es una /suma ponderada de las perturbaciones/. Si supiéramos qué forma tiene la distribución de las $U_i$, sabríamos qué forma tiene la de $\hat\beta_2$. Eso es exactamente lo que vamos a hacer: suponer una forma para $U$ y leer la consecuencia.
