...
Conviene señalar también que la lección 11 ya nos dio, sin que lo pareciera, la pieza que hoy resultará decisiva. Allí escribimos $\hat\beta_2=\beta_2\cdot\mathit1+\sum_iW_iU_i$, y esa descomposición no era un paso intermedio prescindible: dice que el error de estimación $\hat\beta_2-\beta_2$ es una /suma ponderada de las perturbaciones/. Si supiéramos qué forma tiene la distribución de las $U_i$, sabríamos qué forma tiene la de $\hat\beta_2$. Eso es exactamente lo que vamos a hacer: suponer una forma para $U$ y leer la consecuencia.
...
\[
\hat\beta_2=\beta_2\cdot\mathit1+\sum_iW_iU_i.
\]
...
\]
Los pesos $W_i$ son funciones de $\boldsymbol X$, de modo que, una vez que condicionamos en $\boldsymbol X$, se comportan como números fijos —es exactamente el Lema que tanto usamos en la lección 11—. Y entonces $\sum_iW_iU_i$ es, condicionalmente, una combinación lineal de variables normales independientes con coeficientes constantes. Aquí usamos un resultado de su curso de probabilidad que no vamos a demostrar: /una combinación lineal de variables normales independientes es normal/. Con ello, condicionando en $\boldsymbol X$, la distribución de $\hat\beta_2$ es normal, y su media y su varianza son las que ya calculamos en la lección 11:
\[
...
\]
Es decir: el cateto explicado del triángulo de la lección 8 es, exactamente, el regresor en desviaciones multiplicado por $\hat\beta_2$. Esto es propio de la regresión simple —con dos o más regresores no constantes el ajuste en desviaciones ya no está alineado con ningún regresor individual, como advertimos en la lección 10—, y es la razón por la que hoy trabajamos con $k=2$.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
/Respuesta correcta: 2./
/Evalúa/: honestidad epistemológica sobre el alcance de la sesión. Lo demostrado es la parte geométrica, con material del propio curso; lo enunciado es la parte probabilística, que exigiría herramientas ($\chi^2$, distribuciones de direcciones uniformes) que este curso no desarrolla.
