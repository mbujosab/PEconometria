...
#+LANGUAGE: es
#+DESCRIPTION: Lección 2: vectores en R^n, suma, producto por escalar, producto escalar y norma. El lenguaje geométrico con el que el curso construye la regresión.
...
- ([[https://mbujosab.github.io/PEconometria/Transparencias/S02-Lecc02.slides.html][slides]]) --- ([[https://mbujosab.github.io/PEconometria/Lecciones-html/S02-Lecc02.html][html]]) --- ([[https://mbujosab.github.io/PEconometria/Lecciones-pdf/S02-Lecc02.pdf][pdf]])

# --- ([[https://mybinder.org/v2/gh/mbujosab/PEconometria/gh-pages?labpath=CuadernosElectronicos/S02-Lecc02.ipynb][mybinder]]) # comentado porque no estoy exportando las lecciones a .ipynb
...
La lección de hoy es enteramente matemática. No veremos econometría, no veremos datos, no veremos regresión. Veremos vectores como /listas de números/ y las operaciones que se hacen con ellos componente a componente. La recompensa de esta inversión inicial llegará pronto: a partir de la lección 4, las fórmulas de la estadística y de la regresión empezarán a aparecer como consecuencias naturales de la geometría que hoy introducimos.
...
El vector $\boldsymbol{y} = (1, 4, 2, 5, 7) \in \mathbb{R}^5$ es la lista $(1, 4, 2, 5, 7)$. No podemos dibujar $\boldsymbol{y}$ porque no tenemos cómo representar visualmente $\mathbb{R}^5$ (con sus cinco ejes perpendiculares de coordenadas), pero podemos operar con él /igual/ que con $\boldsymbol{x}$.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
Dado $\boldsymbol{x} = (x_1, \dots, x_n)$ y un número real $\alpha$ (llamado /escalar/), su producto es el vector cuyas componentes son las de $\boldsymbol{x}$ multiplicadas por $\alpha$:
\[
\alpha \boldsymbol{x} = (\alpha x_1,\ \alpha x_2,\ \dots,\ \alpha x_n).
\]
Si $\alpha > 1$, el vector resultante señala un punto más lejano en la misma dirección; si $0 < \alpha < 1$, uno más cercano; si $\alpha = 0$, obtenemos el vector cero; si $\alpha < 0$, el vector resultante apunta en sentido contrario.
...
- Tiene /elemento neutro/, el número uno: $1\boldsymbol{x} = \boldsymbol{x}$.
...
- Vídeo: [[https://www.youtube.com/watch?v=k7RM-ot2NWY][Linear combinations, span, and basis vectors]] de 3Blue1Brown (capítulo 2). 12 minutos.
...
Para $\boldsymbol{x}, \boldsymbol{y} \in \mathbb{R}^n$ (espacio euclídeo) el producto escalar _usual_ es:
\[
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
Las dos operaciones que acabamos de ver (suma y producto por escalar) permiten /combinar/ vectores, pero no nos dicen nada sobre /las relaciones geométricas/ entre ellos: cuánto miden, qué ángulo forman, si son perpendiculares. Para abordar estas cuestiones necesitamos una operación nueva, el /producto escalar/.

El producto escalar usual[fn::más adelante veremos que hay otros productos escalares; por ejemplo, en estadística se usa: ${\langle\boldsymbol{x}, \boldsymbol{y}\rangle}_s=\frac{1}{n}{\langle\boldsymbol{x}, \boldsymbol{y}\rangle}_e$.], ${\langle\boldsymbol{x}, \boldsymbol{y}\rangle}_e$,  de dos vectores $\boldsymbol{x}, \boldsymbol{y} \in \mathbb{R}^n$ (también llamado /producto punto/, /producto interior/ estándar o, en inglés, /dot product/) es el número real que se obtiene multiplicando sus componentes correspondientes y sumando todos los productos:
\[
{\langle\boldsymbol{x}, \boldsymbol{y}\rangle}_e = \boldsymbol{x} \cdot \boldsymbol{y} = x_1 y_1 + x_2 y_2 + \cdots + x_n y_n = \sum_{i=1}^n x_i y_i.
\]
Atención a un punto importante de notación: aunque el símbolo ``$\cdot$'' se parece al de la multiplicación de números, /el producto escalar de dos vectores es un número real, no un vector/. Toma como entrada dos listas de números y devuelve un único número. Por eso se llama ``escalar'' (de /escala/, número): el resultado vive en $\mathbb{R}$, no en $\mathbb{R}^n$. Un último aviso sobre el punto: lo nombro aquí, y solo aquí, porque es la notación que encontrará en la mayoría de los textos; pero la notación de trabajo de este curso será siempre $\langle\cdot,\cdot\rangle$ con su subíndice. El motivo es que el subíndice recuerda /con qué convenio estamos midiendo/, y esa será la clave de todo el curso: la misma estructura geométrica reaparecerá en el espacio euclídeo usual (subíndice $e$), al operar como en estadística dividiendo por $n$ (subíndice $s$) y, más adelante (lección 9), al trabajar con variables aleatorias.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
El producto escalar tiene cuatro propiedades fundamentales, todas ellas demostrables aplicando componente a componente las propiedades de la suma y el producto por escalares en $\mathbb{R}$:

1. /Simetría/: $\langle\boldsymbol{x},\, \boldsymbol{y}\rangle = \langle\boldsymbol{y},\, \boldsymbol{x}\rangle$. El orden no importa.
2. /Linealidad/ en cada argumento: para todo $\alpha, \beta \in \mathbb{R}$ y todos $\boldsymbol{x}, \boldsymbol{y}, \boldsymbol{z} \in \mathbb{R}^n$,
   \[
   \big\langle(\alpha \boldsymbol{x} + \beta \boldsymbol{y}),\, \boldsymbol{z}\big\rangle = \alpha \langle\boldsymbol{x},\, \boldsymbol{z}\rangle + \beta \langle\boldsymbol{y},\,\boldsymbol{z}\rangle.
   \]
   Por simetría, lo mismo vale en el segundo argumento. Esta propiedad es la que permitirá, más adelante, manipular expresiones como $\langle(\boldsymbol{y} - \boldsymbol{\mathop{\widehat{y}}}),\,\boldsymbol{x}\rangle$ y separarlas en sumandos manejables.
3. /Positivo/: $\langle\boldsymbol{x},\,\boldsymbol{x}\rangle \geq 0$. Es decir, el producto escalar de un vector consigo mismo es siempre no negativo. Por ejemplo, con el producto escalar usual $\langle\boldsymbol{x},\,\boldsymbol{x}\rangle_e = \sum_{i=1}^n x_i^2 \geq 0$.
4. /Definido/: $\langle\boldsymbol{x},\,\boldsymbol{x}\rangle = 0$ si y solo si $\boldsymbol{x} = \boldsymbol{0}$. Es decir, solo se anula para el vector cero.

Las propiedades tercera y cuarta nos permiten definir una /norma/ a partir del producto escalar, como veremos en la sección siguiente.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
Sean $\boldsymbol{x} = (3, -1, 2, 4) \in \mathbb{R}^4$ y $\boldsymbol{y} = (1, 2, 0, -1) \in \mathbb{R}^4$. Su producto escalar usual es:
\[
\langle\boldsymbol{x},\,\boldsymbol{y}\rangle_e = 3 \cdot 1 + (-1) \cdot 2 + 2 \cdot 0 + 4 \cdot (-1) = 3 - 2 + 0 - 4 = -3.
\]
...
*** Norma o longitud de un vector: \(\|\boldsymbol{x}\|=\sqrt{\langle\boldsymbol{x}, \boldsymbol{x}\rangle}\)
:PROPERTIES:
:metadata: (slideshow . ((slide_type . skip)))
:UNNUMBERED: notoc
:END:
...
/Definición a partir del producto escalar/.[fn:: */Sobre la notación/*: el título de la transparencia escribe la norma de
...
positividad definida). El producto escalar euclídeo ${\langle\boldsymbol{x},\boldsymbol{y}\rangle}_e$
...
escribimos $\|\boldsymbol{x}\|_e=\sqrt{{\langle\boldsymbol{x},\boldsymbol{x}\rangle}_e}$, con
...
\]
La raíz cuadrada está bien definida porque, por la propiedad de positividad, $\langle\boldsymbol{x},\, \boldsymbol{x}\rangle \geq 0$.

En el caso particular del producto escalar usual tenemos:
\[
\|\boldsymbol{x}\|_e = \sqrt{{\langle\boldsymbol{x},\,\boldsymbol{x}\rangle}_e} = \sqrt{x_1^2 + x_2^2 + \cdots + x_n^2}.
\]
...
- En $\mathbb{R}^2$ :: un vector $\boldsymbol{x} = (x_1, x_2)$ corresponde al punto del plano cuyas coordenadas son $(x_1, x_2)$. Sus componentes son los catetos del triángulo rectángulo cuya hipotenusa va del origen a ese punto. Por Pitágoras:
  \[
  \|\boldsymbol{x}\|_e^2 = x_1^2 + x_2^2 \quad\Longrightarrow\quad \|\boldsymbol{x}\|_e = \sqrt{x_1^2 + x_2^2} = \sqrt{{\langle\boldsymbol{x},\,\boldsymbol{x}\rangle}_e}.
  \]
...
- En $\mathbb{R}^3$ :: el vector $\boldsymbol{y} = (y_1, y_2, y_3)$ se proyecta primero sobre el plano horizontal: esa proyección tiene longitud $\sqrt{y_1^2 + y_2^2}$. La proyección de $\boldsymbol{y}$ en el plano horizontal y la componente vertical $y_3$ forman un nuevo triángulo rectángulo cuya hipotenusa es $\boldsymbol{y}$. Aplicando Pitágoras dos veces: 
  \[
  \|\boldsymbol{y}\|_e^2 = \Big(\sqrt{y_1^2 + y_2^2}\Big)^2 + y_3^2 = y_1^2 + y_2^2 + y_3^2 \quad\Longrightarrow\quad \|\boldsymbol{y}\|_e = \sqrt{y_1^2 + y_2^2 + y_3^2} = \sqrt{{\langle\boldsymbol{y},\,\boldsymbol{y}\rangle}_e}.
  \]

- En $\mathbb{R}^n$ :: para $n > 3$ no podemos dibujar, pero /extendemos la definición por analogía/: la misma expresión $\sqrt{{\langle\boldsymbol{x},\,\boldsymbol{x}\rangle}_e}=\sqrt{\sum x_i^2}$ vale en cualquier dimensión. Esta extensión no es arbitraria: es la que sale del producto escalar, y por eso conserva el teorema de Pitágoras en cualquier dimensión (lección 3).

Una nota sobre el rigor matemático seguido en esta segunda lección. En la lección 3, una vez introducida la /ortogonalidad/, demostraremos el teorema de Pitágoras en $\mathbb{R}^n$ sin necesidad de ningún dibujo. Aquí lo usamos como andamio intuitivo, dado por conocido en bajas dimensiones; allí lo probaremos con plena generalidad.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
Para el vector $\boldsymbol{x} = (1, -2, 2, 4)$ y el producto escalar usual:
\[
\|\boldsymbol{x}\|_e = \sqrt{1^2 + (-2)^2 + 2^2 + 4^2} = \sqrt{1 + 4 + 4 + 16} = \sqrt{25} = 5.
\]
La longitud de este vector de $\mathbb{R}^4$ es exactamente $5$. No podemos visualizarla, pero podemos calcularla con la misma soltura que en $\mathbb{R}^2$.
...
Para entenderlo, piense por un momento en cómo medimos /distancias/ en la vida real. Entre dos puntos de una ciudad podemos hablar de:
...
La norma que hemos definido es la /norma euclídea/, derivada del producto escalar usual, y es la elección por defecto. Pero en estadística aparece de manera natural otra forma de medir, que difiere de la euclídea por un factor de $1/n$:
\[
\|\boldsymbol{x}\|_s= \sqrt{{\langle\boldsymbol{x},\,\boldsymbol{x}\rangle}_s} = \sqrt{\frac{1}{n} \sum_{i=1}^n x_i^2}.
\]
...
No vamos a usar esta segunda norma de manera sistemática hasta la lección 4, cuando hagamos el puente entre la geometría y los conceptos estadísticos (media, varianza, covarianza). Pero conviene saber, desde ahora, que la dualidad existe. Explica, por ejemplo, una rareza que algún día le llamará la atención: en los textos de econometría, el coeficiente $R^2$ se escribe a veces como un cociente de /sumas de cuadrados/ ($\mathrm{SEC}/\mathrm{STC}$) y otras veces como un cociente de /varianzas/. La equivalencia entre ambas formulaciones se debe precisamente a que las dos normas (la euclídea y la estadística) difieren solo en un factor multiplicativo que /se cancela al hacer el cociente/.

Quédese con la idea de fondo: /medir/ no es algo único; es una elección que se hace en función del problema. La euclídea es la opción natural en geometría; la estadística, con su factor $1/n$, es la opción natural cuando los componentes del vector son observaciones que queremos resumir en promedio.
...
Próxima lección: /ángulo/ entre vectores, /ortogonalidad/ y el teorema de Pitágoras en $\mathbb{R}^n$, con demostración. Con esto quedará completo el lenguaje geométrico del curso.
...
4. La /norma euclídea/ de un vector es la raíz cuadrada del producto escalar del vector consigo mismo. Coincide en $\mathbb{R}^2$ y $\mathbb{R}^3$ con la longitud que da el teorema de Pitágoras y la extiende, por la misma fórmula, a $\mathbb{R}^n$. Aquí damos ese teorema por conocido; en la lección 3 lo demostraremos en $\mathbb{R}^n$ con rigor.
...
* Preguntas de repaso (sesión 2)                                   :htmlonly:
   :PROPERTIES:
   :metadata: (slideshow . ((slide_type . skip)))
   :END:
#+attr_ipynb: (slideshow . ((slide_type . skip)))
...
*** Comentario
   ...
   :END:
#+attr_ipynb: (slideshow . ((slide_type . skip)))
Las preguntas más valiosas pedagógicamente son la 6 (fuerza a recordar que el producto escalar /no/ es un vector), la 8 (norma del vector $\boldsymbol{1}$, que prepara la lección 4), la 11 (justificación de la doble norma) y la 12 (combinación lineal, que se usará constantemente a partir de la lección 6). Conviene repasarlas con cuidado, especialmente la 11, que conecta la matemática con la motivación estadística que veremos más adelante.
...
1) Un segmento orientado con módulo, dirección y sentido.
2) Una lista ordenada de $n$ números reales.
3) Una matriz cuadrada de orden $n$.
4) Una función definida en $\{1, 2, \dots, n\}$ con valores complejos.
...
1) Sumar dos vectores de $\mathbb{R}^5$.
2) Multiplicar un vector de $\mathbb{R}^7$ por el escalar $\sqrt{2}$.
3) Sumar un vector de $\mathbb{R}^3$ con uno de $\mathbb{R}^4$.
4) Calcular el producto escalar de dos vectores de $\mathbb{R}^{100}$.
...
*** Pregunta 5 [B] — Producto escalar
   ...
   :END:
#+attr_ipynb: (slideshow . ((slide_type . skip)))
Sean $\boldsymbol{x} = (1, 2, 3)$ e $\boldsymbol{y} = (4, 0, -1)$. Su producto escalar /usual/ es:
...
1) Un vector de $\mathbb{R}^n$.
2) Un vector de $\mathbb{R}^{2n}$.
3) Un número real.
4) Una matriz $n \times n$.
...
1) $1$ siempre, independientemente de $n$.
2) $n$.
3) $\sqrt{n}$.
4) $n^2$.
...
1) $\|\boldsymbol{x}\| = \langle\boldsymbol{x},\,\boldsymbol{x}\rangle$.
2) $\|\boldsymbol{x}\|^2 = \langle\boldsymbol{x},\,\boldsymbol{x}\rangle$.
3) $\|\boldsymbol{x}\| = \sum x_i$.
4) $\|\boldsymbol{x}\|^2 = \sum |x_i|$.
...
1) Su norma euclídea vale $1$ y su norma estadística depende de $n$.
2) Su norma euclídea vale $\sqrt{n}$ y su norma estadística vale $1$, independientemente de $n$.
3) Ambas normas valen $n$.
4) Ambas normas valen $1$.
