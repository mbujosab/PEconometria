...
3. /Positivo/: $\langle\boldsymbol{x},\,\boldsymbol{x}\rangle \geq 0$. Es decir, el producto escalar de un vector consigo mismo es siempre no negativo. Por ejemplo, con el producto escalar usual $\langle\boldsymbol{x},\,\boldsymbol{x}\rangle_e = \boldsymbol{x} \cdot \boldsymbol{x} = \sum_{i=1}^n x_i^2 \geq 0$.
...
\[
\langle\boldsymbol{x},\,\boldsymbol{y}\rangle_e = \boldsymbol{x} \cdot \boldsymbol{y} = 3 \cdot 1 + (-1) \cdot 2 + 2 \cdot 0 + 4 \cdot (-1) = 3 - 2 + 0 - 4 = -3.
\]
...
positividad definida). El producto escalar euclídeo $\boldsymbol{x}\cdot\boldsymbol{y}$
...
\[
\|\boldsymbol{x}\|_e = \sqrt{\boldsymbol{x} \cdot \boldsymbol{x}} = \sqrt{x_1^2 + x_2^2 + \cdots + x_n^2}.
\]
...
- En $\mathbb{R}^2$ :: un vector $\boldsymbol{x} = (x_1, x_2)$ corresponde al punto del plano cuyas coordenadas son $(x_1, x_2)$. Sus componentes son los catetos del triángulo rectángulo cuya hipotenusa va del origen a ese punto. Por Pitágoras:
  \[
  \|\boldsymbol{x}\|_e^2 = x_1^2 + x_2^2 \quad\Longrightarrow\quad \|\boldsymbol{x}\|_e = \sqrt{x_1^2 + x_2^2} = \sqrt{\boldsymbol{x}\cdot\boldsymbol{x}}.
  \]
...
- En $\mathbb{R}^3$ :: el vector $\boldsymbol{y} = (y_1, y_2, y_3)$ se proyecta primero sobre el plano horizontal: esa proyección tiene longitud $\sqrt{y_1^2 + y_2^2}$. La proyección de $\boldsymbol{y}$ en el plano horizontal y la componente vertical $y_3$ forman un nuevo triángulo rectángulo cuya hipotenusa es $\boldsymbol{y}$. Aplicando Pitágoras dos veces: 
  \[
  \|\boldsymbol{y}\|_e^2 = \Big(\sqrt{y_1^2 + y_2^2}\Big)^2 + y_3^2 = y_1^2 + y_2^2 + y_3^2 \quad\Longrightarrow\quad \|\boldsymbol{y}\|_e = \sqrt{y_1^2 + y_2^2 + y_3^2} = \sqrt{\boldsymbol{y}\cdot\boldsymbol{y}}.
  \]

- En $\mathbb{R}^n$ :: para $n > 3$ no podemos dibujar, pero /extendemos la definición por analogía/: la misma expresión $\sqrt{\boldsymbol{x}\cdot\boldsymbol{x}}=\sqrt{\sum x_i^2}$ vale en cualquier dimensión. Esta extensión no es arbitraria: es la única que preserva las propiedades de la longitud en $\mathbb{R}^2$ y $\mathbb{R}^3$ (positividad, escalado, desigualdad triangular).
...
1) $\|\boldsymbol{x}\| = \boldsymbol{x} \cdot \boldsymbol{x}$.
2) $\|\boldsymbol{x}\|^2 = \boldsymbol{x} \cdot \boldsymbol{x}$.
