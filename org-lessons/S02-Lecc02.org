...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
Para el vector $\boldsymbol{x} = (3, -1, 2, 4)$ del ejemplo anterior y el producto escalar usual:
\[
\|\boldsymbol{x}\|_e = \sqrt{3^2 + (-1)^2 + 2^2 + 4^2} = \sqrt{9 + 1 + 4 + 16} = \sqrt{30} \approx 5{,}48.
\]
