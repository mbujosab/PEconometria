...
- En $\mathbb{R}^n$ :: para $n > 3$ no podemos dibujar, pero /extendemos la definición por analogía/: la misma expresión $\sqrt{{\langle\boldsymbol{x},\,\boldsymbol{x}\rangle}_e}=\sqrt{\sum x_i^2}$ vale en cualquier dimensión. Esta extensión no es arbitraria: es la única que preserva las propiedades de la longitud en $\mathbb{R}^2$ y $\mathbb{R}^3$ (positividad, escalado, desigualdad triangular).
