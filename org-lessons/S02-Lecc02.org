...
3. /Definido positivo/: $\langle\boldsymbol{x},\,\boldsymbol{x}\rangle \geq 0$, con igualdad si y solo si $\boldsymbol{x} = \boldsymbol{0}$. Es decir, el producto escalar de un vector consigo mismo es siempre no negativo, y solo se anula para el vector cero. Por ejemplo, con el producto escalar usual $\langle\boldsymbol{x},\,\boldsymbol{x}\rangle_e = \boldsymbol{x} \cdot \boldsymbol{x} = \sum_{i=1}^n x_i^2 \geq 0$.
