...
4) $\boldsymbol{x}\cdot\boldsymbol{y} = \|\boldsymbol{x}\|\|\boldsymbol{y}\|$.
...
2) Garantiza que el cociente $\frac{\boldsymbol{a}\cdot\boldsymbol{b}}{\|\boldsymbol{a}\|\|\boldsymbol{b}\|}$ toma valores en $[-1,1]$, donde el coseno está bien definido.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
/Respuesta correcta: 2./
/Evalúa/: conexión entre el teorema de Pitágoras y la ortogonalidad mediante cálculo. $\boldsymbol{x}\cdot\boldsymbol{y} = 1\cdot4 + 2\cdot(-2) = 0$, luego son ortogonales y se cumple Pitágoras. La opción 4) confunde la igualdad de normas (desigualdad triangular con igualdad) con Pitágoras.
