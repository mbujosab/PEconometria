...
#+LANGUAGE: es
#+DESCRIPTION: Lección 3: ángulo entre vectores, ortogonalidad, teorema de Pitágoras en R^n, desigualdades de Cauchy-Schwarz y triangular, y distancia entre vectores.
...
- Programa:
  ...
  3. /Teorema de Pitágoras en $\mathbb{R}^n$/: demostración con rigor y sin dibujo.
...
- $\cos\theta = -1$: ángulo de $180^\circ$ (sentidos opuestos).
...
#+attr_ipynb: (slideshow . ((slide_type . fragment)))
La respuesta es la fórmula siguiente:
\[
...
\(
\quad
\boxed{\;\cos\theta = \Big\langle\frac{\boldsymbol{a}}{\|\boldsymbol{a}\|},\,\frac{\boldsymbol{b}}{\|\boldsymbol{b}\|}\Big\rangle = \dfrac{\langle\boldsymbol{a}\,,\boldsymbol{b}\rangle}{\|\boldsymbol{a}\|\,\|\boldsymbol{b}\|}\;}
\)

#+begin_center
La fórmula es consecuencia del teorema de Pitágoras.
#+end_center
...
/Caso unitario/ (cuando $\|\boldsymbol{a}\| = \|\boldsymbol{b}\| = 1$). Sea $x$ la longitud de la proyección de $\boldsymbol{b}$ sobre la dirección de $\boldsymbol{a}$ (la longitud del segmento $X$ de la figura de la derecha). Por definición de coseno en el triángulo rectángulo, $\cos\theta = x$. Llamando $h$ a la altura del segmento $H$ y $c$ a la longitud del lado opuesto del triángulo (es decir, a la longitud del segmento $C$, que coincide con la norma del vector diferencia: $c = \|\boldsymbol{b}-\boldsymbol{a}\|$); aplicando Pitágoras en los dos triángulos rectángulos obtenemos las ecuaciones:
\[
...
\[
c^2 = \|\boldsymbol{a}-\boldsymbol{b}\|^2 = \langle\boldsymbol{a}\,,\boldsymbol{a}\rangle - 2\langle\boldsymbol{a}\,,\boldsymbol{b}\rangle + \langle\boldsymbol{b}\,,\boldsymbol{b}\rangle = 2 - 2\langle\boldsymbol{a}\,,\boldsymbol{b}\rangle,
\]
donde hemos usado $\|\boldsymbol{a}\| = \|\boldsymbol{b}\| = 1$. Sustituyendo:
\[
\cos\theta = x = \frac{2 - (2 - 2\langle\boldsymbol{a}\,,\boldsymbol{b}\rangle)}{2} = \langle\boldsymbol{a}\,,\boldsymbol{b}\rangle.
\]
...
\[
\cos\theta = \Big\langle\frac{\boldsymbol{a}}{\|\boldsymbol{a}\|},\,\frac{\boldsymbol{b}}{\|\boldsymbol{b}\|}\Big\rangle = \frac{\langle\boldsymbol{a},\,\boldsymbol{b}\rangle}{\|\boldsymbol{a}\|\,\|\boldsymbol{b}\|}.
\]
...
La ortogonalidad es la noción que vertebra el curso. Cuando lleguemos a la regresión, la condición que define el ajuste por mínimos cuadrados será precisamente una condición de ortogonalidad: el vector de residuos[fn::La diferencia entre los datos originales objeto de estudio y el ajuste que el modelo logra para dichos datos.] será ortogonal a los regresores.[fn::Los vectores que se combinan para construir el ajuste. Los definiremos con precisión al llegar a la regresión.] Por eso conviene fijar bien esta definición ahora.
...
\big\langle(\boldsymbol{x}+\boldsymbol{y}),\,(\boldsymbol{x}+\boldsymbol{y})\big\rangle = 
...
\[\|\boldsymbol{x}+\boldsymbol{y}\|^2 = \|\boldsymbol{x}\|^2 + 2\langle\boldsymbol{x},\,\boldsymbol{y}\rangle + \|\boldsymbol{y}\|^2\] aparecerá más veces en el curso.
# +end_center
...
En la lección 2 justificamos que la norma mide la longitud, con Pitágoras clásico en $\mathbb{R}^2$ y $\mathbb{R}^3$. Ahora, con la noción de ortogonalidad que acabamos de definir, podemos ir más lejos: demostrar el teorema en $\mathbb{R}^n$ sin recurrir a ningún dibujo.
...
\begin{aligned}
\|\boldsymbol{x}+\boldsymbol{y}\|^2 &= \big\langle(\boldsymbol{x}+\boldsymbol{y}),\,(\boldsymbol{x}+\boldsymbol{y})\big\rangle && \text{(definición de norma)}\\
&= \langle\boldsymbol{x},\,\boldsymbol{x}\rangle + \langle\boldsymbol{x},\,\boldsymbol{y}\rangle + \langle\boldsymbol{y},\,\boldsymbol{x}\rangle + \langle\boldsymbol{y},\,\boldsymbol{y}\rangle && \text{(linealidad en ambos argumentos)}\\
&= \langle\boldsymbol{x},\,\boldsymbol{x}\rangle + 2\langle\boldsymbol{x},\,\boldsymbol{y}\rangle + \langle\boldsymbol{y},\,\boldsymbol{y}\rangle && \text{(simetría: } \langle\boldsymbol{x},\,\boldsymbol{y}\rangle = \langle\boldsymbol{y},\,\boldsymbol{x}\rangle)\\
&= \|\boldsymbol{x}\|^2 + 2\langle\boldsymbol{x},\,\boldsymbol{y}\rangle + \|\boldsymbol{y}\|^2. && \text{(definición de norma)}
\end{aligned}

Esta igualdad es válida /siempre/, sean o no ortogonales los vectores. De ella se lee directamente el teorema: la igualdad $\|\boldsymbol{x}+\boldsymbol{y}\|^2 = \|\boldsymbol{x}\|^2 + \|\boldsymbol{y}\|^2$ se cumple si y solo si el término central se anula, es decir, si y solo si $\langle\boldsymbol{x},\,\boldsymbol{y}\rangle = 0$, esto es, si y solo si $\boldsymbol{x}\perp\boldsymbol{y}$. $\blacksquare$
...
\[
\langle\boldsymbol{a},\,\boldsymbol{h}\rangle = 0 \;\Longrightarrow\; \boxed{\alpha = \frac{\langle\boldsymbol{a},\,\boldsymbol{b}\rangle}{\langle\boldsymbol{a},\,\boldsymbol{a}\rangle}}.
\]
...
\[
0 = \langle\boldsymbol{a},\,\boldsymbol{h}\rangle = \Big\langle\boldsymbol{a},\,(\boldsymbol{b}-\alpha\boldsymbol{a})\Big\rangle = \langle\boldsymbol{a},\,\boldsymbol{b}\rangle - \alpha\langle\boldsymbol{a},\,\boldsymbol{a}\rangle \implies \boxed{\alpha = \frac{\langle\boldsymbol{a},\,\boldsymbol{b}\rangle}{\langle\boldsymbol{a},\,\boldsymbol{a}\rangle}}.
\]
...
\[
\|\boldsymbol{b}\|^2 > \alpha^2\|\boldsymbol{a}\|^2 = \frac{\langle\boldsymbol{a},\,\boldsymbol{b}\rangle^2}{\langle\boldsymbol{a},\,\boldsymbol{a}\rangle^2}\,\|\boldsymbol{a}\|^2 = \frac{\langle\boldsymbol{a},\,\boldsymbol{b}\rangle^2}{\Big(\|\boldsymbol{a}\|^2\Big)^2}\,\|\boldsymbol{a}\|^2 = \frac{\langle\boldsymbol{a},\,\boldsymbol{b}\rangle^2}{\|\boldsymbol{a}\|^2}.
\]
Multiplicando por $\|\boldsymbol{a}\|^2$ obtenemos $\|\boldsymbol{a}\|^2\|\boldsymbol{b}\|^2 > \langle\boldsymbol{a},\,\boldsymbol{b}\rangle^2$, y tomando raíces, $\|\boldsymbol{a}\|\|\boldsymbol{b}\| > |\langle\boldsymbol{a},\,\boldsymbol{b}\rangle|$.
...
$\boldsymbol{h}=\boldsymbol{0}$, es decir, $\|\boldsymbol{h}\|^2 = 0$; y entonces $\|\boldsymbol{b}\|^2 = \alpha^2\|\boldsymbol{a}\|^2$. Consecuentemente $|\langle\boldsymbol{a},\,\boldsymbol{b}\rangle| = \|\boldsymbol{a}\|\|\boldsymbol{b}\|$.
Así, la desigualdad es estricta cuando los vectores *no* están alineados, y se convierte en igualdad cuando lo están. (Si $\boldsymbol{a}=\boldsymbol{0}$ no hay nada que probar: ambos lados de la desigualdad valen $0$. Ese caso queda fuera de la construcción anterior porque $\alpha$ exige dividir por $\langle\boldsymbol{a},\,\boldsymbol{a}\rangle$, que entonces se anula.)

#+begin_quote
/Idea para recordar/. El vector $\alpha\boldsymbol{a}$, con $\alpha = \frac{\langle\boldsymbol{a},\,\boldsymbol{b}\rangle}{\langle\boldsymbol{a},\,\boldsymbol{a}\rangle}$, es la ``sombra'' de $\boldsymbol{b}$ sobre la dirección de $\boldsymbol{a}$. 
...
más adelante será también la pendiente de la regresión lineal simple (cuando $\boldsymbol{a}$ son los datos centrados del regresor $\boldsymbol{x}$).
#+end_quote
...
\[
\cos\theta = \frac{\langle\boldsymbol{a},\,\boldsymbol{b}\rangle}{\|\boldsymbol{a}\|\,\|\boldsymbol{b}\|}, \qquad -1 \leq \cos\theta \leq 1.
\]
...
- /Vectores alineados/: cuando $\boldsymbol{b} = \lambda\boldsymbol{a}$ con $\lambda\neq 0$. En ese caso el ángulo es $0$ (si $\lambda > 0$, ``apuntan'' en el mismo sentido) o $180^\circ$ (si $\lambda < 0$), porque
  \[
  \cos\theta = \frac{\lambda\,\|\boldsymbol{a}\|^2}{|\lambda|\,\|\boldsymbol{a}\|^2} = \frac{\lambda}{|\lambda|} = \pm 1.
  \]
  Los vectores alineados son el caso opuesto a los ortogonales: máxima dependencia frente a máxima independencia direccional. Esta imagen será útil en la lección 14, cuando hablemos de /colinealidad/: dos regresores muy correlacionados son vectores /casi/ alineados (ángulo pequeño, coseno cercano a $\pm 1$), y eso hará inestable la regresión.
...
\[
\|\boldsymbol{a}+\boldsymbol{b}\|^2 = \|\boldsymbol{a}\|^2 + 2\langle\boldsymbol{a},\,\boldsymbol{b}\rangle + \|\boldsymbol{b}\|^2 \leq \big(\|\boldsymbol{a}\| + \|\boldsymbol{b}\|\big)^2.
\]
...
/Demostración/. Partimos del cuadrado del binomio vectorial (la identidad que destacamos en ``Teorema de Pitágoras en $\mathbb{R}^n$''):

\begin{aligned}
\|\boldsymbol{a}+\boldsymbol{b}\|^2 &= \|\boldsymbol{a}\|^2 + 2\langle\boldsymbol{a},\,\boldsymbol{b}\rangle + \|\boldsymbol{b}\|^2 \\
&\leq \|\boldsymbol{a}\|^2 + 2\,\|\boldsymbol{a}\|\,\|\boldsymbol{b}\| + \|\boldsymbol{b}\|^2 && \text{(por Cauchy–Schwarz: } \langle\boldsymbol{a},\,\boldsymbol{b}\rangle \leq |\langle\boldsymbol{a},\,\boldsymbol{b}\rangle| \leq \|\boldsymbol{a}\|\|\boldsymbol{b}\|)\\
...
Esta noción de distancia es, literalmente, el corazón de la regresión: en la lección 6 buscaremos, de entre todos los vectores de un subespacio, el vector $\boldsymbol{\mathop{\widehat{y}}}$ que /minimiza la distancia/ $\|\boldsymbol{y} - \boldsymbol{\mathop{\widehat{y}}}\|$ al vector de datos $\boldsymbol{y}$. Ajustar por mínimos cuadrados será, exactamente, minimizar una distancia $\|\boldsymbol{y}-\boldsymbol{\mathop{\widehat{y}}}\|$, donde $\boldsymbol{\mathop{\widehat{y}}}$ es el vector ajustado (aún no definido formalmente; lo haremos en la lección 6).
...
#+end_export
| Concepto          | Fórmula matemática / definición algebraica                                                                                                        |
|-------------------+---------------------------------------------------------------------------------------------------------------------------------------------------|
...
1. La /longitud/, justificada en la lección 2 con Pitágoras clásico en $\mathbb{R}^2$ y $\mathbb{R}^3$, se extiende por analogía a $\mathbb{R}^n$.
...
* Preguntas de repaso (sesión 3)                                   :htmlonly:
   :PROPERTIES:
   :metadata: (slideshow . ((slide_type . skip)))
   :END:
#+attr_ipynb: (slideshow . ((slide_type . skip)))
...
/Evalúa/: definición de ortogonalidad. La opción 4) confunde ortogonalidad con opuesto.
...
/Evalúa/: enunciado del teorema. La opción 1) es el caso de /igualdad/ de la desigualdad triangular, que solo se da cuando los vectores están alineados y con el mismo sentido. La opción 3) es equivalente a 1), no a 2).
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
/Respuesta correcta: 2./
/Evalúa/: cálculo directo. $\langle\boldsymbol{a},\,\boldsymbol{b}\rangle = 3$, $\|\boldsymbol{a}\| = 3$, $\|\boldsymbol{b}\| = \sqrt{2}$, luego $\cos\theta = \frac{3}{3\sqrt{2}} = \frac{1}{\sqrt{2}}$.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
/Respuesta correcta: 3./
/Evalúa/: cálculo de $\|\boldsymbol{a}\| = \sqrt{4} = 2$ y división. La opción 2) sería correcta si $\|\boldsymbol{a}\| = \sqrt{2}$, lo que requeriría solo dos componentes iguales a $1$.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
/Respuesta correcta: 3./
/Evalúa/: la identidad es consecuencia de las propiedades del producto escalar y vale siempre. El teorema de Pitágoras es su caso especial: cuando $\langle\boldsymbol{x},\,\boldsymbol{y}\rangle=0$, el término central desaparece.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
/Respuesta correcta: 2./
/Evalúa/: conexión entre el teorema de Pitágoras y la ortogonalidad mediante cálculo. $\langle\boldsymbol{x},\,\boldsymbol{y}\rangle = 1\cdot4 + 2\cdot(-2) = 0$, luego son ortogonales y se cumple Pitágoras. La opción 4) confunde la igualdad de normas (desigualdad triangular con igualdad) con Pitágoras.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
/Respuesta correcta: 4./
/Evalúa/: vectores alineados con escalar negativo. $\cos\theta = \frac{\langle\boldsymbol{a},\,(-2\boldsymbol{a})\rangle}{\|\boldsymbol{a}\|\cdot2\|\boldsymbol{a}\|} = \frac{-2\|\boldsymbol{a}\|^2}{2\|\boldsymbol{a}\|^2} = -1$. Discrimina porque la tentación es responder $-2$ (el escalar), confundiendo el escalar con el coseno.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
/Respuesta correcta: 3./
/Evalúa/: conexión con las lecciones siguientes. Si $\boldsymbol{a} = \boldsymbol{1}$, entonces $\alpha = \frac{\langle\boldsymbol{1},\,\boldsymbol{b}\rangle}{\langle\boldsymbol{1},\,\boldsymbol{1}\rangle} = \frac{\sum b_i}{n} = \mu_{\boldsymbol{b}}$. Discrimina porque exige haber seguido el hilo del guiño explícito de la lección.
