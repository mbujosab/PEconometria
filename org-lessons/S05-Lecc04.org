...
\[
\langle\boldsymbol{x},\boldsymbol{y}\rangle_{s} = \frac{1}{n}\,\langle\boldsymbol{x},\boldsymbol{y}\rangle_{e} = \frac{1}{n}\sum_{i=1}^n x_i y_i.
\]
...
\]
donde el /vector de medias/ es un vector constante: $\boldsymbol{\mathop{\overline{y}}}=\mu_{\boldsymbol{y}}\boldsymbol{1}$; es decir, $\boldsymbol{\mathop{\overline{y}}}\in\mathcal{L}(\boldsymbol{1})$; 
...
La segunda figura de esta sección ilustra las dos posibilidades con /dos/ vectores de media cero distintos, $\boldsymbol{x}$ y $\boldsymbol{z}$: el vector $\boldsymbol{y}+\boldsymbol{x}$ tiene mayor desviación típica que $\boldsymbol{y}$, mientras que $\boldsymbol{y}+\boldsymbol{z}$ tiene exactamente la misma. Los tres vectores, en cambio, tienen exactamente la misma proyección sobre $\mathcal{L}(\boldsymbol{1})$ (la misma media).
...
2) Sí: $\|\boldsymbol{x}\|_s^2 = 5$, $\mu^2 + \sigma^2 = 4 + 1 = 5$.
...
2) Porque la media es el valor $c$ que minimiza $\frac{1}{n}\sum(y_i - c)^2$, es decir, $\|\boldsymbol{y} - c\boldsymbol{1}\|_s^2$.
...
2) $\langle\boldsymbol{y} - \alpha\boldsymbol{1},\, \boldsymbol{1}\rangle_s = 0$, es decir, $(\boldsymbol{y} - \alpha\boldsymbol{1}) \perp \boldsymbol{1}$.
3) $\|\boldsymbol{y}\|_s^2 = \|\alpha\boldsymbol{1}\|_s^2$.
4) $\alpha = \|\boldsymbol{y}\|_s$.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
/Respuesta correcta: 2./
/Evalúa/: verificación de la identidad pitagórica. $\|\boldsymbol{x}\|_s^2 = \frac{1}{2}(1+9) = 5$.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
/Respuesta correcta: 3./
/Evalúa/: comprensión de que el factor $1/n$ hace que $\|\boldsymbol{1}\|_s = 1$ independientemente de $n$. $\|\boldsymbol{1}\|_s^2 = \frac{1}{4}(1+1+1+1) = 1$.
