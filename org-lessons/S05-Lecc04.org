...
#+LANGUAGE: es
#+DESCRIPTION: Lección 4: la media aritmética como proyección ortogonal sobre el vector de unos y la desviación típica como norma del vector en desviaciones.
...
$\boldsymbol{b}$, es decir, $\alpha\boldsymbol{a}$ es el punto de $\mathcal{L}(\boldsymbol{a})$ que minimiza $\|\boldsymbol{b} - \alpha\boldsymbol{a}\|$.
...
Conviene dar nombre también a lo que queda fuera de la recta. El /complemento ortogonal/ de $\mathcal{L}(\boldsymbol{1})$, que denotamos $\mathcal{L}(\boldsymbol{1})^\perp$, es el conjunto de todos los vectores ortogonales a $\boldsymbol{1}$; es decir, el conjunto de vectores de media nula (recuerde que $\mu_{\boldsymbol{v}}=\langle\boldsymbol{v},\boldsymbol{1}\rangle_s$): $\boldsymbol{v}\in\mathcal{L}(\boldsymbol{1})^\perp \Leftrightarrow \mu_{\boldsymbol{v}}=0$.
...
#+CAPTION: Dos lecturas de la media, lado a lado, con $n=10$ datos (dos muestras distintas, arriba y abajo). Izquierda: histograma con la media marcada (lectura estadística: el centro de la distribución). Derecha: el mismo vector de datos, componente a componente, junto al vector de medias (constante, en rojo), que es la proyección sobre $\mathcal{L}(\boldsymbol{1})$ (lectura geométrica).
...
#+attr_ipynb: (slideshow . ((slide_type . fragment)))
Las figuras de hoy tienen su [[https://mybinder.org/v2/gh/mbujosab/PEconometria/gh-pages?labpath=CuadernosElectronicos/S05-Notebook01.ipynb][versión interactiva]] en $\mathbb{R}^3$: rote los vectores y compruebe que la desviación típica es la norma del vector en desviaciones.
...
Geométricamente, $a\boldsymbol{1}$ está en $\mathcal{L}(\boldsymbol{1})$, así que sumarlo cambia solo la componente de $\boldsymbol{y}$ en esa recta y deja intacta la componente perpendicular. La desviación típica, que es la norma de esa componente perpendicular, no varía.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
El mensaje central de esta sesión es que la media $\mu$ y la desviación típica $\sigma$, dos de los estadísticos más elementales, son objetos geométricos: la primera es el coeficiente de la proyección ortogonal sobre $\boldsymbol{1}$ (el escalar que multiplica a $\boldsymbol{1}$) y la segunda es una norma.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
/Respuesta correcta: 2./
/Evalúa/: conexión entre la condición de primer orden del cálculo y la condición de ortogonalidad. Discrimina porque exige haber seguido la doble derivación (analítica y geométrica) de la transparencia ``La media: dos caras de la misma moneda''.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
/Respuesta correcta: 3./
/Evalúa/: comprensión del patrón general (una condición por regresor) anticipado al final de la transparencia ``El vector en desviaciones es ortogonal a $\boldsymbol{1}$''. Discrimina porque requiere haber asimilado el guiño al futuro.
