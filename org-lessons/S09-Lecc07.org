...
#+LANGUAGE: es
#+DESCRIPTION: Lección 7: estimación MCO sin matrices. Las dos condiciones de ortogonalidad dan la pendiente (covarianza sobre varianza) y la constante de la recta ajustada.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
La primera condición de ortogonalidad, $\langle\boldsymbol{\mathop{\widehat{e}}} , \boldsymbol{1}\rangle_s=0$ ya la hemos visto en otro contexto: es la misma condición que, en la lección 4, hacía que el vector en desviaciones tuviera suma cero. Aquí el residuo es más complejo, pues depende de dos coeficientes y no de uno, pero la condición algebraica es idéntica: la media de los residuos es cero, por tanto, /la suma de los residuos es cero/.
...
#+attr_ipynb: (slideshow . ((slide_type . fragment)))
Sustituyendo $\hat\beta_1=\mu_{\boldsymbol{y}}-\hat\beta_2\mu_{\boldsymbol{x}}$ (transparencia ``Primera condición: el residuo tiene media cero'') y agrupando:
\[
...
#+attr_ipynb: (slideshow . ((slide_type . fragment)))
Como $\langle\boldsymbol{x},\boldsymbol{y}\rangle_s=\mu_{\boldsymbol{x}\odot\boldsymbol{y}}$ (media del producto componente a componente):
\[
...
Sustituimos ahora $\hat\beta_1=\mu_{\boldsymbol{y}}-\hat\beta_2\mu_{\boldsymbol{x}}$ (transparencia ``Primera condición: el residuo tiene media cero'') y agrupamos los términos con $\hat\beta_2$:
\[
...
Falta reconocer el lado izquierdo; y resulta ser la ``fórmula de cálculo'' de la /covarianza/, el análogo exacto de la fórmula de la varianza con dos vectores distintos en lugar de uno consigo mismo. En efecto, por la misma bilinealidad que hemos usado en toda esta lección (y que ya usamos en la lección 2):
\[
...
\]
que es ya, prácticamente, el resultado final. Solo falta despejar $\hat\beta_2$, lo que hacemos en la transparencia ``$\hat\beta_2$: covarianza sobre varianza''.
...
#+attr_ipynb: (slideshow . ((slide_type . fragment)))
#+begin_center
La pendiente MCO es /la correlación, reescalada por el cociente de las desviaciones típicas/: su signo es el de $\rho_{\boldsymbol{x}\boldsymbol{y}}$.
#+end_center
...
- Wooldridge, J. M. (2020), cap. 2, sección 2.2: fórmula de la pendiente y comentario sobre su signo.
...
- Strang, G. (2016). /Introduction to Linear Algebra/, sección 4.4 (/Orthonormal Bases and Gram-Schmidt/): bases ortogonales y proyecciones independientes.
...
#+attr_ipynb: (slideshow . ((slide_type . fragment)))
Tres cantidades que coinciden: $\mu_{\boldsymbol{y}}=\mu_{\boldsymbol{\mathop{\widehat{y}}}}=\hat\beta_1+\hat\beta_2\mu_{\boldsymbol{x}}$ ([[https://mybinder.org/v2/gh/mbujosab/PEconometria/gh-pages?labpath=CuadernosElectronicos/S09-Notebook02.ipynb][versión interactiva]]).
...
Es más: en opinión del autor de estos apuntes, *sin constante no puede hablarse de regresión*, porque nos salimos del marco de la estadística; no incluirla es un error conceptual. El laboratorio que sigue a la lección 8 muestra qué se pierde al omitirla.

/Para profundizar/:
- Wooldridge, J. M. (2020), cap. 2, sección 2.3: propiedades algebraicas de MCO, entre ellas que la recta pasa por el punto de las medias.
...
Como en la [[https://mbujosab.github.io/PEconometria/Practicas-pdf/S07-Prct-B-verificacion-geometrica.pdf#page=3][actividad 3]] ([[https://mbujosab.github.io/PEconometria/Practicas-html/S07-Prct-B-verificacion-geometrica.html#relacion-conocida][html]]) del laboratorio ($\boldsymbol{y}=2+3\boldsymbol{x}+\boldsymbol{u}$), pero con $n=5$, calculable a mano:
...
#+attr_ipynb: (slideshow . ((slide_type . fragment)))
/Versión interactiva/: [[https://mybinder.org/v2/gh/mbujosab/PEconometria/gh-pages?labpath=CuadernosElectronicos/S09-Notebook02.ipynb][cuaderno 2 en MyBinder]], parte 3 (qué le exigimos a $\boldsymbol{u}$ para recuperar los coeficientes).
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
Verifiquemos con números todo lo dicho hasta ahora. Construimos un ejemplo pequeño, calculable enteramente a mano, en el mismo espíritu que la actividad de la práctica de laboratorio ([[https://mbujosab.github.io/PEconometria/Practicas-pdf/S07-Prct-B-verificacion-geometrica.pdf#page=3][actividad 3]] ([[https://mbujosab.github.io/PEconometria/Practicas-html/S07-Prct-B-verificacion-geometrica.html#relacion-conocida][html]])), donde se generaban datos a partir de una relación lineal conocida $\boldsymbol{y}=2+3\boldsymbol{x}+\boldsymbol{u}$ para comprobar que MCO recupera aproximadamente esos coeficientes generadores. Aquí, con solo $n=5$ observaciones (frente a la muestra mayor y aleatoria de la práctica), podemos seguir el cálculo paso a paso.
