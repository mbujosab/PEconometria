...
Además, sobre la recta azul y sobre los segmentos en desviaciones verá
pequeñas cruces: son una regla graduada. Pase el ratón por encima de
cualquiera de ellas para ver el valor exacto que representa ($\mu$ o
$\sigma$, según el caso), y cuéntelas desde el origen (o desde la
proyección) para leerlos directamente en la figura, igual que contaba
diagonales en las cuadrículas de papel de la lección 4.

Todas las figuras de las lecciones 4 y 5 son necesariamente esquemáticas ($\mathbb{R}^2$ o $\mathbb{R}^3$ dibujados en el papel). Aquí podrá /rotar/ las figuras de verdad con el ratón (arrastrando sobre ellas) y /cambiar los vectores/ editando una celda y volviéndola a ejecutar.
...
Pase el ratón sobre cualquier cruz para ver su valor exacto.
...
Edite las componentes de $\boldsymbol{y}$ y vuelva a ejecutar la celda para ver una figura nueva.
...
Arrastre sobre la figura para rotarla. Observe:
...
- el segmento punteado que une $\boldsymbol{\mathop{\overline{y}}}$ con $\boldsymbol{y}$: es el
  ...
  plano gris, sino /paralelo/ a él: ese plano gris es
  $\mathcal{L}(\boldsymbol{1})^\perp$, que pasa por el origen, y el segmento
  es una copia de un vector de ese plano trasladada al punto
  $\boldsymbol{\mathop{\overline{y}}}$);
- las pequeñas cruces sobre la recta azul y sobre el segmento punteado:
  cuéntelas desde el origen hasta $\boldsymbol{\mathop{\overline{y}}}$ para leer
  $\mu_{\boldsymbol{y}}$ (con signo), y desde $\boldsymbol{\mathop{\overline{y}}}$ hasta $\boldsymbol{y}$
  para leer $\sigma_{\boldsymbol{y}}$; pase el ratón por encima de cualquiera
...
$\sigma_{\boldsymbol{y}}$ (compruébelo pasando el ratón por encima). La celda
siguiente solo confirma numéricamente lo que ya puede leer contando
cruces en la figura.
...
Cambie $\boldsymbol{y}$ (por ejemplo, a un vector con media negativa, como $(2,-1,-7)$) y vuelva a ejecutar las dos celdas anteriores. Observe cómo cambia la posición de $\boldsymbol{\mathop{\overline{y}}}$ sobre la recta azul (¿a qué lado del origen queda cuando $\mu_{\boldsymbol{y}}<0$?), y compruebe que la identidad de Pitágoras se sigue cumpliendo.

* Parte 2: dos vectores — el ángulo y la correlación

/(Cobra la geometría de la lección 5.)/

Retomamos el ejemplo numérico de la lección 5: $\boldsymbol{x}=(1,2,3)$ y $\boldsymbol{v}=(2,2,5)$, con $\rho_{\boldsymbol{v}\boldsymbol{x}}=\frac{\sqrt3}{2}$ (un ángulo de $30^\circ$: ni ortogonales, ni alineados).
...
Rote la figura hasta colocar el punto de vista casi sobre el plano gris
...
cuántas cruces caben en cada vector trasladado, puede leer también, a
...
Modifique $\boldsymbol{v}$ y $\boldsymbol{x}$ y vuelva a ejecutar las celdas anteriores para explorar los casos extremos:
- Vectores en desviaciones /ortogonales/ ($\rho=0$, $\theta=90^\circ$): por ejemplo, pruebe con $\boldsymbol{x}=(1,2,3)$ y $\boldsymbol{v}=(2,-1,2)$.
- Vectores en desviaciones /alineados/ ($\rho=\pm1$): por ejemplo, $\boldsymbol{x}=(1,2,3)$ y $\boldsymbol{v}=2\boldsymbol{x}$ (¿y si toma $\boldsymbol{v}=-2\boldsymbol{x}$?).

En ambos casos, compruebe en la figura que el resultado coincide con lo que predice la geometría: en el primer caso, los vectores discontinuos trasladados al origen deben verse perpendiculares; en el segundo, deben verse sobre la misma recta (misma dirección, o dirección opuesta).

* Para seguir explorando

- El parámetro =rango= de =figura_R3= controla el tamaño de la escena (auméntelo si sus vectores tienen componentes grandes).
- Puede activar la rejilla de puntos de coordenadas enteras con =mostrar_reticula_puntos=True= al llamar a =figura_R3=; por defecto está desactivada porque con muchos puntos la figura se recarga visualmente.
- Vuelva a la lección 4 para releer la definición de $\boldsymbol{\mathop{\overline{y}}}$ y $\sigma_{\boldsymbol{y}}$, y a la lección 5 para la covarianza y la correlación como coseno de un ángulo.
