...
Podrá, como en el primer cuaderno, rotar las figuras con el ratón y editar los vectores en las celdas de código para crear sus propios ejemplos. Además, junto a cada figura 3D verá su gemela: el diagrama de dispersión clásico $(x_i,y_i)$ con la recta ajustada, para no perder de vista la lectura “de manual” de lo mismo.
...
def ajuste_mco_simple(y, x):
    """Ajuste MCO de la regresión simple y = beta1*1 + beta2*x (lección 7).
    Devuelve (yhat, ehat, beta1, beta2)."""
    ...
    beta2 = sigma_xy / sigma_x2
    beta1 = mu_y - beta2 * mu_x
    yhat = beta1 + beta2 * x
    ehat = y - yhat
    return yhat, ehat, beta1, beta2
...
def agrega_componentes_ajuste(fig, beta1, beta2, x, yhat):
    """Dibuja beta1*1 y beta2*x, y los segmentos punteados del
    paralelogramo que, sumados, reconstruyen yhat."""
    x = np.asarray(x, dtype=float)
    uno = np.ones_like(x)
    comp0 = beta1 * uno
    comp1 = beta2 * x
    agrega_vector(fig, comp0, color='darkorange', nombre='beta1*1', dash='dot', width=10)
    agrega_vector(fig, comp1, color='sienna', nombre='beta2*x', dash='dot', width=10)
...
def figura_ajuste_R3(x, y, rango=5, titulo="", mostrar_componentes=True):
    ...
    y = np.asarray(y, dtype=float)
    n = len(x)
    yhat, ehat, beta1, beta2 = ajuste_mco_simple(y, x)
    ...
    if mostrar_componentes:
        agrega_componentes_ajuste(fig, beta1, beta2, x, yhat)
...
def diagrama_dispersion(x, y, mostrar_residuos=False, titulo=""):
    ...
    ax.plot(xs, beta1 + beta2 * xs, color='crimson',
            label=f"yhat = {beta1:.2f} + {beta2:.2f}*x")
    if mostrar_residuos:
...
Un pequeño diccionario de pares $(\boldsymbol{x},\boldsymbol{y})$ mejor o peor ajustados: =canonico= es el ejemplo de la lección 5 ($\rho_{\boldsymbol{x}\boldsymbol{y}}=\sqrt3/2$); =fuerte= tiene una correlación muy alta; =debil=, muy baja. Cámbielos, o añada el suyo, para repetir cualquier actividad del cuaderno con otros datos.
...
#+BEGIN_SRC jupyter-python
yhat, ehat, beta1, beta2 = ajuste_mco_simple(y, x)
print(f"beta1_hat = {beta1:.4f}   beta2_hat = {beta2:.4f}")
...
Cambie al ejemplo =fuerte= o =debil= (o cree el suyo) y vuelva a ejecutar las tres celdas anteriores. Observe cómo cambia la /longitud/ del segmento gris (el residuo) y compárela con lo que verá en el diagrama de dispersión: cuanto más “pegados” estén los puntos a la recta, más corto será ese segmento.

* Parte 2: los vectores en desviaciones, en $\mathcal{L}(\boldsymbol{1})^\perp$

Recordemos la identidad guardada en la lección 7: el vector en desviaciones del ajuste coincide con el de $\hat\beta_2\boldsymbol{x}$,
\[
...
Añadimos también, en azul, la propia recta $\mathcal{L}(\boldsymbol{1})$: aunque no pertenece a este plano —es, de hecho, /perpendicular/ a él—, sirve como referencia para orientar la figura. Si gira esta figura hasta que $\mathcal{L}(\boldsymbol{1})$ apunte directamente hacia usted (saliendo de la pantalla), estará mirando el plano $\mathcal{L}(\boldsymbol{1})^\perp$ “de frente”; intente poner la figura de la Parte 1 en esa misma orientación y compruebe que ambas coinciden.
...
print("beta2 * (x - x_barra):", np.round(beta2 * ex, 4))
print("¿son el mismo vector? ", np.allclose(eyhat, beta2 * ex))
#+END_SRC

*** Actividad

Repita esta figura con el ejemplo =debil=. Fíjese en el ángulo entre el vector verde y el violeta/negro: debería verse mucho más abierto que con el ejemplo =canonico=. Este ángulo, y su coseno al cuadrado, serán exactamente el $R^2$ del próximo cuaderno.
...
#+BEGIN_SRC jupyter-python
def experimento_u(x, beta1, beta2, u, nombre=""):
    """Genera y = beta1*1 + beta2*x + u, ajusta MCO y devuelve un dict
    con los resultados, listo para comparar con los valores verdaderos."""
    x = np.asarray(x, dtype=float)
    u = np.asarray(u, dtype=float)
    uno = np.ones_like(x)
    y = beta1 * uno + beta2 * x + u
    yhat, ehat, beta1_hat, beta2_hat = ajuste_mco_simple(y, x)
...
print(f"Valores verdaderos: beta1 = {beta1}, beta2 = {beta2}\n")
resultados = []
for nombre, u in casos:
    res = experimento_u(x, beta1, beta2, u, nombre)
...
fig_b = figura_ajuste_R3(x, res_b['y'], rango=6,
    titulo="Caso (b): u en L(1,x)^perp -- recuperacion exacta de beta1,beta2")
fig_b.show()
#+END_SRC
...
Elija su propio vector $\boldsymbol{u}$ (o modifique alguno de los cinco anteriores) y compruebe en qué categoría cae —calculando $\sum u_i$ y $\boldsymbol{u}\cdot\boldsymbol{x}$— antes de ejecutar =experimento_u=. Intente encontrar un $\boldsymbol{u}\neq\boldsymbol{0}$ que, sin estar exactamente en la dirección $\boldsymbol{1}\times\boldsymbol{x}$, recupere aproximadamente bien los coeficientes: ¿qué tiene de especial, comparado con los que sí lo consiguen exactamente?

* Para seguir explorando

- El parámetro =rango= controla el tamaño de la escena; aumente su valor si sus vectores tienen componentes grandes (como en el caso (b) o (e) de la Parte 3).
...
- El laboratorio que sigue a la lección 8 comprobará las mismas dos ortogonalidades, pero con datos reales.
