...
Repita ambas celdas con los ejemplos =fuerte= y =debil=. Observe cómo, al pasar de =fuerte= a =debil=, el ángulo $\theta$ se abre hacia $90^\circ$ y $R^2$ se acerca a $0$ —los “casos extremos” de la lección 8—, y compruebe en cada caso que $R^2=\cos^2\theta=\rho_{\boldsymbol{x}\boldsymbol{y}}^2$ sigue cumpliéndose.
...
def cuadrados_ajuste(x, y, titulo_general=""):
    ...
    ax = axes[1]
    ax.scatter(x, y, color='seagreen', zorder=3)
    ax.plot(xs, beta1 + beta2 * xs, color='crimson', zorder=2)
    for xi, yi, yhi in zip(x, y, yhat):
    ...
    ax.axhline(mu_y, color='blue', linewidth=1)
    ax.plot(xs, beta1 + beta2 * xs, color='crimson', alpha=0.3, zorder=1)
...
for nombre, u in [("u = 0 (sin perturbacion)", u_cero),
    ...
    print(nombre)
    print(f"   beta1_hat = {beta1_hat:.4f}   beta2_hat = {beta2_hat:.4f}   R^2 = {R2:.4f}\n")
#+END_SRC

Con $\boldsymbol{u}=\boldsymbol{0}$, $R^2=1$ (ajuste perfecto, hipotenusa y cateto coinciden). Con $\boldsymbol{u}$ ortogonal al plano pero /no/ nulo, $\hat\beta_1,\hat\beta_2$ siguen siendo exactamente $2$ y $3$ —MCO “acierta” con los coeficientes generadores—, pero $R^2<1$: hay un residuo real, $\boldsymbol{\mathop{\widehat{e}}}=\boldsymbol{u}$, y el ajuste, aunque tiene los coeficientes correctos, no reproduce los datos exactamente ($\boldsymbol{\mathop{\widehat{y}}}\ne\boldsymbol{y}$).

#+BEGIN_SRC jupyter-python
y_perp = beta1 * uno + beta2 * x + u_perp
...
Multiplique =u_perp= por un factor mayor (por ejemplo, =5 * np.cross(uno, x)=) y repita la comparación. Compruebe que $\hat\beta_1,\hat\beta_2$ siguen siendo exactamente $2$ y $3$ —porque la dirección de $\boldsymbol{u}$ no ha cambiado, solo su tamaño—, mientras que $R^2$ disminuye a medida que $\boldsymbol{u}$ crece. Relacione esta observación con la transparencia “Interpretando $R^2$: casos extremos” de la lección 8.
...
- =cuadrados_ajuste= funciona con cualquier ejemplo; pruébelo con los cinco vectores $\boldsymbol{u}$ de la Parte 3 del Notebook 2 para ver, de un vistazo, cómo cambian las tres áreas.
