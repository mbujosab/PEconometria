...
/Comandos nuevos de esta práctica/. Casi todo se ha visto ya: =open=, =ols= y sus accesores =$sigma=, =$stderr=, =$rsq=, =$ess=, =$coeff= y =$df= (sesiones 11 a 18), =critical= (sesión 18) y =$vcv= (práctica B de la sesión 21). Recuerde que los accesores se refieren siempre al /último/ modelo estimado: por eso, en esta práctica, se guardan en escalares antes de estimar el siguiente. Lo nuevo:
- =vif= :: se escribe después de estimar un modelo con al menos dos regresores no constantes, y para cada regresor calcula $1/(1-R_j^2)$, con $R_j^2$ el de la regresión de ese regresor sobre los demás. En la ventana del modelo es //~Análisis -> Colinealidad~//.
- =$vcv[3,4]= :: =$vcv= es la matriz de varianzas y covarianzas de los coeficientes, y entre corchetes van la fila y la columna. Los coeficientes siguen el orden de la lista de regresores, así que =[3,4]= es la covarianza entre el tercero (=nox=) y el cuarto (=lnox=), y =[3,3]= la varianza del tercero.
...
Dos regresiones seguidas. La primera es el modelo completo: guardamos en =s= la desviación típica de la regresión, $\mathfrak s$ (=$sigma=), y en =ee_nox= el error estándar de =nox=. La segunda es la /regresión auxiliar/: =nox= como regresando sobre la constante, =rooms= y =lnox=. De ella guardamos el $R^2$ y la raíz de su $\mathrm{SRC}$ (=$ess=), que es la longitud de la parte propia de =nox=.
...
Tres =printf= con los escalares guardados: $\mathfrak s$ entre la longitud de la parte propia, el error estándar que imprimió Gretl, y el VIF con su raíz, calculados a partir del $R^2$ auxiliar.
...
Hay que volver a estimar el modelo completo (con =--quiet=, para no repetir la tabla), porque =vif= actúa sobre el último modelo estimado y el último fue la regresión auxiliar.
...
La misma secuencia con el modelo sin =lnox=: estimación, $\mathfrak s$ guardado en =s_sin=, regresión auxiliar de =nox= sobre =rooms= (ahora un solo regresor), el cociente impreso directamente con los accesores de esa auxiliar, y =vif= tras volver a estimar el modelo de precios.
...
Con el modelo completo estimado, =critical(t, $df, 0.025)= da el valor crítico de la $t$ con los grados de libertad del modelo (sesión 18), y cada intervalo es el coeficiente más y menos =tc= por su error estándar. En el =printf=, =%%= imprime un signo de tanto por ciento (un =%= solo abriría un hueco). La correlación entre los dos estimadores es su covarianza dividida por la raíz del producto de sus varianzas, las tres leídas en =$vcv=.
