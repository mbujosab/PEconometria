...
#+LANGUAGE: es
#+DESCRIPTION: Práctica de Gretl, sesión 7 (C): cuatro series con la misma media y desviación típica y formas distintas; histogramas, diagramas de caja, asimetría y curtosis.
#+OPTIONS: toc:nil
#+OPTIONS: date:nil
...
/Comandos nuevos de esta práctica/. Seguimos usando =nulldata=, =set seed=, =normal=, =uniform=, =series=, =scalar= y =printf= de las prácticas A y B. Lo nuevo:
- =setobs 1 1 --cross-section= :: declara que las observaciones son de sección cruzada (sesión 4).
- =D_bruto[1] = 15= :: cambia solo la primera componente de la serie; los corchetes seleccionan una observación.
- =(uniform(0,1) < 0.5)= :: una comparación aplicada a una serie produce otra serie de ceros y unos, con un uno donde se cumple la condición. Sirve para sortear a qué grupo pertenece cada empleado.
- =sd(x)= :: desviación típica de la serie, con divisor $n-1$ (véase el Apéndice). =median=, =skewness= y =kurtosis= se presentan en la Actividad 5.
- =summary --simple= :: versión reducida de =summary=: media, mediana, desviación típica, mínimo y máximo.
- =boxplot A B C D= :: un diagrama de caja por variable, en la misma figura y con la misma escala. =freq= dibuja el histograma de una variable (sesión 4).

* Actividad 1 - Generación de los datos

Generamos cuatro series de $n=40$ observaciones con distribuciones muy distintas, pero forzando en las cuatro la misma media ($\mu_0$) y la misma desviación típica ($\sigma_0$, medida con la función =sd= de Gretl; véase la nota sobre el divisor en el Apéndice). El truco (reescalado de un vector en desviaciones) usa exactamente las propiedades de invarianza y homogeneidad demostradas en la lección 5; el detalle completo se explica en el Apéndice.

Conjunto vacío de 40 observaciones, declarado de sección cruzada, semilla fija y tres escalares: =n=, el número de observaciones (=$nobs=), y los dos valores objetivo, la media =mu0= y la desviación típica =sigma0=. El texto que sigue a =#= es un comentario que Gretl ignora.
...
Tres pasos, que se repetirán en las cuatro series. =A_bruto=: 40 sorteos de una normal estándar, la forma que queremos. =A_z=: la serie estandarizada, restando su media y dividiendo por su desviación típica, con lo que pasa a tener media $0$ y =sd= igual a $1$. =A=: la serie final, con la media y la desviación típica objetivo, =mu0 + sigma0 * A_z=. Por último, =setinfo= le pone descripción y nombre para los gráficos, como en la sesión 4.
...
Misma receta; cambia solo la forma de partida. =B_bruto= es la suma de los cuadrados de dos normales estándar independientes, una $\chi^2$ con 2 grados de libertad, con cola larga hacia la derecha.
...
La forma de partida es una mezcla de dos grupos. =grupo= vale $1$ o $0$ para cada empleado según un sorteo uniforme, con probabilidad $1/2$ cada valor. =C_bruto= toma en cada fila un valor de una normal centrada en $-3$ si =grupo= vale $1$, y de una normal centrada en $3$ si vale $0$: la fórmula =grupo*normal(-3,1) + (1-grupo)*normal(3,1)= anula uno de los dos sumandos en cada fila. Después, la misma estandarización y el mismo reescalado.
...
Plantilla muy homogénea: normal de media $0$ y desviación típica $0{,}3$. La segunda línea sustituye la primera observación por $15$, cincuenta desviaciones típicas por encima: el directivo. Después, la misma estandarización y el mismo reescalado, que reparten el efecto de ese valor entre la media y la =sd= comunes.
...
=summary= de las cuatro series. Con =--simple= muestra solo media, mediana, desviación típica, mínimo y máximo, una fila por serie.
...
#+begin_src gretl :tangle guiones/S07-Prct-C-momentos-no-bastan.inp :eval no :exports none :noweb yes
...
#+include: ./S07-Prct-C-momentos-no-bastan/estadisticos.txt example
#+latex: }
...
- GUI: seleccione cada variable → clic derecho → //~Gráficos~// → //~Distribución de frecuencias~//. En la consola, =freq= dibuja el histograma de una variable (sesión 4); hace falta una orden por serie.
...
\begin{minipage}{0.24\textwidth}
  \includegraphics[width=\textwidth]{./S07-Prct-C-momentos-no-bastan/histA.png}
\end{minipage}%
\begin{minipage}{0.24\textwidth}
  \includegraphics[width=\textwidth]{./S07-Prct-C-momentos-no-bastan/histB.png}
\end{minipage}%
\begin{minipage}{0.24\textwidth}
  \includegraphics[width=\textwidth]{./S07-Prct-C-momentos-no-bastan/histC.png}
\end{minipage}%
\begin{minipage}{0.24\textwidth}
  \includegraphics[width=\textwidth]{./S07-Prct-C-momentos-no-bastan/histD.png}
\end{minipage}%
\caption{Histogramas.}
\end{figure}
#+END_EXPORT
...
<figure style="display: flex; justify-content: center; gap: 10px;">
    <img src="./S07-Prct-C-momentos-no-bastan/histA.png" style="max-width:24%; height:auto;"/>
    <img src="./S07-Prct-C-momentos-no-bastan/histB.png" style="max-width:24%; height:auto;"/>
    <img src="./S07-Prct-C-momentos-no-bastan/histC.png" style="max-width:24%; height:auto;"/>
    <img src="./S07-Prct-C-momentos-no-bastan/histD.png" style="max-width:24%; height:auto;"/>
    <figcaption>Histogramas.</figcaption>
</figure>
#+END_EXPORT
...
=boxplot= con las cuatro series dibuja las cuatro cajas en una misma figura y con la misma escala vertical. En cada caja, la línea horizontal es la mediana, la cruz es la media y los puntos aislados son los valores atípicos.
...
\begin{minipage}{0.45\textwidth}
  \includegraphics[width=\textwidth]{./S07-Prct-C-momentos-no-bastan/boxplotABCD.png}
\end{minipage}%
...
#+BEGIN_EXPORT HTML
<figure style="display: flex; justify-content: center; gap: 10px;">
    <img src="./S07-Prct-C-momentos-no-bastan/boxplotABCD.png" style="max-width:50%; height:auto;"/>
    <figcaption>Diagramas de caja.</figcaption>
</figure>
#+END_EXPORT
...
El diagrama de caja pone en primer plano, de forma directamente comparable entre las cuatro empresas y sobre una única escala vertical, varios resúmenes que el histograma deja implícitos: la /mediana/; el /rango intercuartílico/, una medida de dispersión robusta que sí difiere entre empresas, a diferencia de la desviación típica, idéntica por construcción en las cuatro series. Lo que hay que mirar aquí es la caja de Consultora Este: es estrechísima (el $50\%$ central de los salarios cabe en unos $56$ euros, frente a más de $230$ en las otras tres empresas) y la media, marcada con una cruz, queda /fuera/ de ella, por encima del tercer cuartil. Eso delata el efecto del directivo con salario disparado: un solo dato arrastra la media lejos del grueso de la plantilla, algo que un histograma con pocos intervalos podría disimular en una barra casi invisible. Y, sobre todo, el /marcado explícito de valores atípicos/ como puntos aislados fuera de los bigotes: en Consultora Este, un único punto extremo salta a la vista de inmediato, mientras que en un histograma ese mismo dato podría pasar desapercibido como una barra solitaria de altura mínima en el extremo del eje. El boxplot es, en definitiva, el instrumento idóneo para comparar /varias/ distribuciones de un vistazo sobre los mismos ejes.
...
Una tabla hecha con =printf=. La primera línea imprime la cabecera: un hueco =%-15s= (texto de quince caracteres, alineado a la izquierda) y cinco huecos =%10s= (diez caracteres, alineados a la derecha). Cada línea siguiente rellena una fila con el nombre de la empresa y cinco funciones aplicadas a su serie: =mean=, =median=, =sd=, =skewness= y =kurtosis=. Los anchos fijos hacen que las columnas queden alineadas.
...
outfile --quiet tablaestadisticosadicionales.txt
    <<Tabla estadisticos adicionales>>
end outfile
#+end_src
  
  
#+latex: {\vspace{0pt} \color{gray!70!black} \footnotesize
#+include: ./S07-Prct-C-momentos-no-bastan/tablaestadisticosadicionales.txt example
#+latex: }
...
Por construcción, $\boldsymbol{z}$ tiene media $0$ y desviación típica $1$ (es el vector en desviaciones de $\boldsymbol{x}$, reescalado por su propia norma estadística). Esto es lo que hicimos en cada serie con =A_z=, =B_z=, =C_z=, =D_z=. Nótese que estandarizar /no cambia la forma/ de la distribución: solo la traslada y la reescala; la asimetría o la bimodalidad de $\boldsymbol{x}$ se conservan intactas en $\boldsymbol{z}$.

#+begin_quote
/Nota técnica sobre Gretl y el divisor./ En esta práctica usamos =sd= tanto para estandarizar como para verificar el resultado final. Por eso, /sea cual sea el convenio interno exacto de Gretl para esa función/,[fn::en concreto, =sd= divide por $n-1$, como se comprueba en la práctica B de esta misma sesión; pero el argumento que sigue no depende de ello.] la operación
\[
...
outfile --quiet verificacionmomentos.txt
    <<Verificacion momentos comunes>>
end outfile
#+end_src
  
  
#+latex: {\vspace{0pt} \color{gray!70!black} \footnotesize
#+include: ./S07-Prct-C-momentos-no-bastan/verificacionmomentos.txt example
#+latex: }
...
Lo mismo para las series estandarizadas =A_z=, =B_z=, =C_z= y =D_z=, que deben tener =sd= igual a $1$ y media $0$.
...
outfile --quiet estandarizacioninterna.txt
    <<Verificacion estandarizacion interna>>
end outfile
#+end_src
  
  
#+latex: {\vspace{0pt} \color{gray!70!black} \footnotesize
#+include: ./S07-Prct-C-momentos-no-bastan/estandarizacioninterna.txt example
#+latex: }
