...
Generamos cuatro series de $n=40$ observaciones con distribuciones muy distintas, pero forzando en las cuatro la misma media ($\mu_0$) y la misma desviación típica ($\sigma_0$). El truco (reescalado de un vector en desviaciones) usa exactamente las propiedades de invarianza y homogeneidad demostradas en la lección 5; el detalle completo se explica en el Apéndice.
...
#+begin_quote
/Nota técnica sobre Gretl y el divisor./ En esta práctica usamos =sd()= tanto para estandarizar como para verificar el resultado final. Por eso, /sea cual sea el convenio interno exacto de Gretl para esa función/,[fn::dividir por $n$ o dividir por $n-1$] la operación
\[
