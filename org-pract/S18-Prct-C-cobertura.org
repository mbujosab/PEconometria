...
- =quantile(x, p)= :: el cuantil de orden =p= de la serie: el valor que deja por debajo una fracción =p= de los datos.
- =%= :: resto de la división entera: =(index - 1) % 12= toma los valores $0,1,\ldots,11$ y vuelve a empezar.
- =rech += expresión= :: suma la expresión al escalar y guarda el resultado en él; es la forma abreviada de =rech = rech + expresión=. Como aquí la expresión vale $1$ o $0$, el escalar cuenta las réplicas que cumplen la condición.
- =smpl 1 12= :: restringe la muestra a las observaciones 1 a 12 (sesión 14); =smpl full= la devuelve entera.
- =gnuplot ... --time-series --with-lines= :: dibuja las series frente al número de observación y con líneas en lugar de puntos.
...
smpl 1 100
series B2 = 0.5
gnuplot cL cR B2 --time-series --with-lines
...
Los dos primeros =printf= comparan la cobertura con =tc= y con $1{,}96$ (medias de =dentro= y de =dentroZ=). Los dos siguientes imprimen los cuantiles empíricos del $2{,}5\%$ y del $97{,}5\%$ de los $4\,000$ estadísticos, calculados con =quantile=, junto a los valores que predicen la $t_{10}$ y la normal. El último calcula la fracción de $|t|$ mayores que $1{,}96$. =freq tstat= dibuja el histograma de los estadísticos.
...
Dos bloques con la misma estructura que los de la Actividad 3, cambiando la perturbación: =randgen(X, 1)= sortea una $\chi^2$ con un grado de libertad, que tiene media $1$ y varianza $2$; restar $1$ y dividir por $\sqrt2$ la centra y le da desviación típica $1$, y multiplicar por =sig= le da la de nuestro mundo. Los contadores son ahora la cobertura y los rechazos de la hipótesis cierta.
