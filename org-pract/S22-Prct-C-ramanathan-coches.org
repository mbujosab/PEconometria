...
/Comandos nuevos de esta práctica/. No hay ninguno: =open= y =labels= (sesión 4), =summary --simple= y el comando =corr= (sesión 7), =ols= y sus accesores (sesiones 11 a 18), =vif= (práctica B de hoy), =restrict= y =$test= (sesión 21), =$vcv= (práctica B de la sesión 21 y de hoy) y =smpl= (sesiones 14 a 18). Dos recordatorios:
- =restrict= ... =end restrict= :: contrasta sobre el último modelo la restricción escrita dentro; =b[miles] = 0= se refiere al coeficiente de =miles=. Con =--quiet= no imprime las estimaciones restringidas, y =$test= recoge después el estadístico del contraste.
- =smpl 1 28= :: restringe la muestra a las observaciones 1 a 28: todo lo que se estime después usa solo esas filas. =smpl full= devuelve la muestra completa. En los menús, //~Muestra -> Establecer rango~//.
...
=open= carga el fichero de muestra de Ramanathan (=--quiet= evita listar las variables) y =labels= muestra la descripción de cada una. =summary --simple= da media, mediana, desviación típica, mínimo y máximo de las tres variables, y el comando =corr= la correlación entre =age= y =miles=. Las dos últimas líneas estiman las dos regresiones simples, cada una con su tabla completa.
...
La regresión con los dos regresores y, a continuación, =vif= sobre ese modelo (práctica B).
...
Regresión auxiliar de =miles= sobre =age= (con =--quiet=, porque solo queremos su =$ess=): la raíz es la longitud de la parte propia de =miles=. Después se vuelve a estimar el modelo completo y se imprime $\mathfrak s$ entre esa longitud junto al error estándar de =miles= que da Gretl; =$sigma= y =$stderr= se refieren al modelo que acaba de estimarse.
...
Sobre el modelo completo, el bloque =restrict= contrasta que el coeficiente de =miles= sea cero, y el $F$ resultante queda en =$test=. La correlación entre los dos estimadores se lee en =$vcv=: la fila 2 y la columna 3 corresponden a =age= y =miles=, segundo y tercer coeficiente de la lista (el primero es la constante). El guion completo añade los intervalos, el $F$ global del pie de la tabla (=$Fstat=) y los valores $p$ con =pvalue= (sesión 18).
...
=smpl 1 28= deja activas las 28 primeras observaciones y la regresión que sigue se estima solo con ellas. =smpl 29 57= hace lo mismo con la segunda mitad, y =smpl full= restablece los 57 coches antes de seguir.
