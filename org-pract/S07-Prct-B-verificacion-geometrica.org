...
Repetir a mano el mismo bloque de código para cada serie es razonable con dos series, pero se volvería tedioso con veinte. Gretl ofrece el comando =loop= precisamente para automatizar este tipo de repeticiones. Como en la Actividad 5, no pretendemos aquí explicar la mecánica de los bucles (=loop=) de Gretl —eso se hará con detalle en la sesión de propiedades de MCO (lección 9) y en las prácticas de las sesiones 17 y 19—; preséntelo simplemente como una =caja negra'' que aplica, serie a serie, exactamente el mismo cálculo que acabamos de hacer a mano. La ventaja se aprecia enseguida: con muy poco código adicional podríamos repetir esta misma comprobación sobre diez, veinte o cien series distintas, sin copiar el bloque una y otra vez.
...
*** Covarianza como producto escalar de centrados
...
#+latex: {\vspace{3pt} \footnotesize \color{gray!70!black}
/Aquí conocemos la "verdad": $y$ se construyó con pendiente 3 respecto a $x$. En la próxima sesión veremos cómo/ recuperar /esa pendiente a partir solo de los datos observados, sin conocerla de antemano (estimación MCO)./
#+latex: }
...
#+latex: {\vspace{3pt} \footnotesize \color{gray!70!black}
/Esta actividad es opcional: es probable que no dé tiempo a cubrirla en clase. Resérvela para si surge la pregunta natural: "¿esto se cumple siempre, o ha sido casualidad de esta muestra concreta?" No se pretende aquí explicar la mecánica de los bucles (=loop=) de Gretl --eso se hará con detalle en la sesión de propiedades de MCO (sesión 12) y en las prácticas de las sesiones 17 y 19--; preséntelo simplemente como una "caja negra" que repite la verificación cientos de veces con una muestra nueva en cada repetición./
#+latex: }
...
#+latex: {\vspace{3pt} \footnotesize \color{gray!70!black}
/Las tres desviaciones máximas deberían ser minúsculas (del orden de $10^{-10}$ o menores), del mismo orden que en una sola muestra (Actividad 1). Esto ilustra que no importa/ cuántos /ejemplos distintos se generen: las identidades se cumplen siempre, no "casi siempre"./
#+latex: }
...
- Cottrell, A. y Lucchetti, R. (2023). /Gretl User's Guide/. Funciones de generación aleatoria (=normal()=, =uniform()=), =set seed=, y estructura básica del comando =loop= (consulta puntual; su uso sistemático se explica en la sesión 12).
